...
ER 図も同じで、`er({ ... })` は既定を持たない (カタログの `er-demo` は `palette: "kinari"` を明記している)。
既定を決めているのは dragon の `配色と縞を当てる` で、そこを通るのは記法と JSON の入口だから。

カタログの図は組み立て器で組んでいるので、`palette:` を書き続ける。
...
カタログは 3 形とも色みを書かない = 書くと種類の既定より勝って 6 本とも同じ色になる。

### 札は線の色に従わない

線に添える札 (`継承` 等) は線とは別の塊にあり、色みが届かない。
札の枠と字は「線」 の色のままにした。
...
### 矢じりの大きさも 2 倍にした (#1966)

線を 7 にした時、輪郭の太さは上げたが矢じりの大きさ (engine の 14) は据え置いていた。
三角は線とほぼ同じ幅になって向きが読めず、白抜きの三角は塗りつぶしに、集約の菱形と依存の開いた矢は点に見えていた。

矢じりの形を線の先端を中心に 2 倍に描き、枠の外へはみ出した分を見せる。
輪郭は拡大で 2 倍になるので、書く値は半分にして画面上の太さを上の表の値に保つ。

| 何 | 書く値 | 倍率 | 画面上の値 |
|---|---|---|---|
| 白抜きの印の輪郭 | 2.18 | 2 | 4.36 |
| 開いた矢と三又の輪郭 | 2.49 | 2 | 4.98 |

ER の端 (`one` / `many` 等) は外した。
engine が既に 1.8 倍の枠で描いており、線の太さに負けていないため。

**全ての図に同じ値を掛けている**。
ER 図も状態遷移図もフローも 7 で、図の種類では分けない。
...
画面で `getComputedStyle` を読んで測っており、クラス図 ・ ER 図 ・ フロー のいずれも 7 で
固定してある。
...
3 案のどれを採っても独立に決まる。
engine は元から持っていたのに、カタログで 1 度も使っていなかった。

カタログの `class-demo` で `Auditable` に `interface`、`User` に `abstract` を出した。
