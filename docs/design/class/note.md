...
線に添える札 (`EXTENDS` 等) は線とは別の塊にあり、色みが届かない。
札の枠と字は「線」 の色のままにした。
...
関係の線を 2.25 から 3.5 にした。
印の輪郭も線との比を保って上げてある。

| 何 | 前 | 後 | 線との比 |
|---|---|---|---|
| 線 | 2.25 | 3.5 | — |
| 白抜きの印の輪郭 | 1.4 | 2.18 | 0.622 |
| 開いた矢の輪郭 | 1.6 | 2.49 | 0.711 |

線だけ太くすると印が痩せて見えるので、比を動かさずに一緒に上げる。
箱の枠 (1.75) との比は 1.29 から 2.0 になった。

**クラス図だけに効かせている**。
§ 関係を色で分ける で 6 種を 3 群の色に分けた結果、色が意味を持つようになったのはクラス図だけ。
細い線は色が薄く見えて群の見分けが効きにくい。
関係が 1 種類の ER 図と状態遷移図には同じ理由が当たらないので、そちらは 2.25 のまま。

### それまでクラス図を名指しできなかった

線の太さは画面の `[data-cdl-role="edge-line"]` 1 箇所で決まる。
engine は絵の根に `data-cdl-stage` と `data-cdl-scale` と `data-cdl-palette` の 3 つしか
出しておらず、図の種類が無かった。

配色でも選び分けられない = クラス図と ER 図がどちらも `kinari` を出すため。

engine が `data-cdl-type` を出すようになって初めて書けた。
書けるようになるまでは、直すと ER 図 ・ 状態遷移図 ・ 流れ図 ・ 並び図 の線まで同時に太くなる。

### 4 案を実物の描画で並べて決めた

2.25 (いま) / 2.75 / 3 / 3.5 を同じ図で見比べて 3.5 を採った。
数字だけでは決まらない = 図を画面の幅に収めると線が 1 画素を切り、太さの差が消える。
ほぼ等倍の帯に切り出して並べている。

画面で `getComputedStyle` を読んで測っており、クラス図 3.5 / ER 図 2.25 / 流れ図 2.25 を
実測で固定してある。
限定を外すと他の図が 3.5 に漏れることも、壊して確かめた。
