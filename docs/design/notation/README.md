...
カタログの図は engine の出力そのままで並び、画面の検査がその見た目を見ている。

| 見る場所 | 図の数 | engine への追随 | 色を見る検査 |
|---|---|---|---|
| catalog | カタログの記法 file が持つ分 | 自動 | あり |
| ここ | 貯めた分だけ | 無し (焼き付け) | 無し |

図が何枚あるかは数えて出す。 `packages/dragon/test/inline-stage-height.test.ts` が落ちた時の
文面に走査した枚数が出るので、そこで読む。 ここに数を書き写すと、図を 1 枚足した日から古くなる。
