...
export const patternPassthrough = diagram("pattern-passthrough", { topic: "pattern: Passthrough (中継 node 貫通)" })
  ...
  .edge("a", "c", { id: "e", label: "Client → Service", sub: "Gateway 経由", tone: "accent", style: "dotted-flow", labelOffsetY: -160 })
...
export const patternBranch = diagram("pattern-branch", { topic: "pattern: Branch (条件分岐)" })
  ...
  .edge("check", "ok", { id: "e2", label: "true", tone: "success", style: "dotted-flow", labelOffsetY: -120 })
  .edge("check", "ng", { id: "e3", label: "false", tone: "error", style: "dotted-flow", labelOffsetY: 120 })
...
export const patternFanIn = diagram("pattern-fan-in", { topic: "pattern: Fan-in (複数 worker → 集約)" })
  ...
  .edge("w1", "agg", { id: "e1", label: "result", tone: "teal", style: "dotted-flow", labelOffsetX: 60, labelOffsetY: -80 })
  .edge("w2", "agg", { id: "e2", label: "result", tone: "teal", style: "dotted-flow", labelOffsetX: 60 })
  .edge("w3", "agg", { id: "e3", label: "result", tone: "teal", style: "dotted-flow", labelOffsetX: 60, labelOffsetY: 80 })
...
export const patternRollback = diagram("pattern-rollback", { topic: "pattern: Rollback (失敗時巻き戻し)" })
  ...
  .edge("op", "db", { id: "e3", label: "tentative write", tone: "warning", style: "dotted-flow", labelOffsetY: 200 })
  .edge("op", "commit", { id: "e4", label: "on error", tone: "error", style: "dotted-flow" })
  .edge("commit", "db", { id: "e5", label: "ROLLBACK", tone: "error", style: "dotted-flow", labelOffsetY: 250 })
...
export const patternValidateProcess = diagram("pattern-validate-process", { topic: "pattern: Validate → Process (検証後処理)" })
  ...
  .edge("validate", "process", { id: "e3", label: "ok", tone: "success", style: "dotted-flow", labelOffsetY: -120 })
  .edge("validate", "err", { id: "e4", label: "ng", tone: "error", style: "dotted-flow", labelOffsetY: 120 })
