...
test.describe("CAR-755 Handdrawn theme quality regression (Round 11 意図 pin)", () => {
  ...
  test("軸 B: 全 node-label が stroke=none + fill=#2c2820 ink + Caveat / Bradley Hand cursive", async ({
    page,
  }) => {
    await page.goto(`${CATALOG_URL}?theme=handdrawn`, { waitUntil: "networkidle" });
    await waitStable(page);
    const result = await page.evaluate(() => {
      ...
      const stats = labels.map((el) => {
        ...
        return {
          ...
          fontCursive: /Caveat|Bradley Hand|Comic Sans|cursive/i.test(cs.fontFamily),
        };
      });
    ...
    // Caveat はウェブフォントで落ちる可能性があるが、 font-family 宣言に含まれることを pin
  ...
  test("軸 E: generic.tsx の g 直下 path/ellipse も sticky yellow surface に override (white fallback 混入検出)", async ({
    ...
    expect(result.total).toBeGreaterThan(0);
    // 全 shape が sticky yellow surface (fill = #fdf7d9 = rgb(253, 247, 217)) で描画
