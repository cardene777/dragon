/**
 * 画面の種類ごとに 1 枚ずつ、明るい画面 / 暗い画面 / 携帯の幅で撮る (#1094)。
 *
 * 元は `tests/audit-3vp.spec.ts` で 33 件 (11 経路 × 3) の検査として置かれていたが、 `expect` を
 * 1 つも持たない。 判定を持たない以上これは検査ではなく目視用の道具なので、 script として持つ。
 *
 * **全 page を撮る道具ではない** (#2244)。 見るのは画面の種類 (表紙 / 説明書 / 編集画面 /
 * 分類の頁 / 見本の頁 / 変更履歴 / 手引き / 見つからない頁) で、下の `ROUTES` はその代表。
 * 分類の頁は見え方が分類ごとに変わらないので数件だけ載せている
 * (全分類へ広げると撮る枚数が分類の数 x 3 倍になり、目視用の道具の長さを超える)。
 * 分類ごとの図を 1 枚ずつ見るのは `shoot-zoom.mjs` の役目で、そちらは分類を画面から拾う。
 *
