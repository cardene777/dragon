 ...
 * ## 幅は 1 つでよい (#2553)
 *
 * 初めは 375 と 390 (iPhone 14) の 2 つで全 38 画面を開いていた。
 * 2 つ置いた理由は書かれておらず、注釈にあるのは機種名だけだった。
 * 測ると、この 2 つは同じことを見ていた。
 *
 * 見た目を変える閾値がこの間に無い。
 * この app の CSS が持つ幅の閾値のうち携帯に近いのは `max-width: 560px` と `620px` で、
 * 375 と 390 はどちらも同じ枝に落ちる。
 *
 * 全 29 経路を両方の幅で開いて中身の右端を測っても、頁のはみ出しは両方で 0 だった。
 * 唯一 390 のほうが外へ出ていた編集画面 (375 は画面の端ちょうど、390 は 35.4px 外) も、
 * 出ていたのは自分で横に送れる枠 (`v4-editor-pan`) の中身で、下の測り方はそれを対象から
 * 外している。
 *
 * 理屈でも狭い側が厳しい。
 * はみ出しの主因はこれ以上縮まない下限を持つ要素で、下限が同じなら狭い側が先に溢れる。
 * 折り返しで広い側だけが溢れる形は成立しない = 折り返しは使える幅より長い行を作らないので、
 * 390 を超える塊は 375 でも超える。
 *
 * **幅を足すなら狭い側へ足す**。
 * 375 より広い幅は、上の 3 つが覆らない限り同じ判定の繰り返しになる。
 */
const 幅一覧 = [375] as const;
