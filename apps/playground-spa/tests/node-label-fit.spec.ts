 ...
 * ある = 文字を書かない形では 35 種が `card` に落ちて絵が測れていなかったが、フローの箱は
...
async function 箱の絵を測る(
  ...
  return await page.evaluate(() => {
    /*
     * **測る前に動きを止めて先頭に戻す** (#1486)。
     *
     * 回っている絵を持つ種別では、外接矩形が **絵ではなく「矩形を回したもの」** になる。
     * 親から見た子の外接矩形は、子の矩形の 4 隅を回してから軸に沿って囲み直した形なので、
     * 円に近い絵でも 45 度で最も膨らむ。
     *
     * 実測 = `shape-gear` は箱 320 に対して回っている間 348.9 (左右へ 13 ずつ) だが、
     * 止めて先頭に戻すと 320 ちょうど。 **絵は 1 度も箱から出ていない** (歯先の直径は
     * 箱の短辺の 0.79 倍)。 止めずに測っていた間は、この膨らみを「はみ出し」 と読んでいた。
     *
     * **戻す先は 0 秒ではなく 5 秒**。 0 秒だと、始まりを遅らせた動き (`begin`) がまだ
     * 当たっておらず、位置を動きだけで決めている部品が原点に残る (実測 = `shape-exchange` の
     * 粒 6 個が箱の左 29 の位置に出た)。 遅らせは最大でも 1 秒台なので、5 秒なら全て始まっている。
     *
     * 5 秒は回る絵にとっても都合がよい。 回転は 20 秒で 1 周なので 5 秒はちょうど 90 度 =
     * 矩形が軸に沿ったままで、上の膨らみが出ない。
     *
     * **回転の途中は測れない**。 外接矩形が絵を表さないので、90 度の倍数以外で外へ出る形は
     * この検査では見ていない。 見るには絵の輪郭そのものを回す必要がある。
     */
    for (const svg of document.querySelectorAll<SVGSVGElement>("svg[data-cdl-stage]")) {
      svg.pauseAnimations();
      svg.setCurrentTime(5);
      /*
       * **止まったことを確かめる** (#1486)。
       *
       * 止めずに時刻だけ合わせても、その瞬間はたまたま 90 度なので測れてしまう。 だが測る間に
       * 進むので、種別を 1 つずつ撮っていくうちに角度がずれて膨らむ = 落ちる回と通る回が
       * 出る。 止め忘れがこの形で紛れ込むと「たまに落ちる検査」 になるので、ここで弾く。
       */
      if (!svg.animationsPaused()) throw new Error("図の動きを止められていない");
    }
    const svg = document
 ...
 * 無くなりフローの箱は 320×150 になるので、書かないと帯が下限を割らず落ちない (実測)。
 *
