 ...
 *
 * 縮む側と伸びる側の実寸は `packages/dragon/test/support/responsive-accepted.ts` の
 * `代表的な図` が持ち、`readability-representative.test.ts` が描き直して突き合わせる
 * (同じ表を複数の file に書くと片方だけ古くなる)。 枚数も書かない = 図を足すたびに動く
 * (同じ file の `受け入れた一覧` と `縦に長い一覧` が名前で持つ)。
 *
