...
/**
 * 仕様書が **わざと数を書かない** 節。 1 件ずつ理由を書く (#1836)。
 *
 * 上の一覧から黙って落とすと、 数の主張が消えたのか一覧に足し忘れたのかを読み手が
 * 分けられない。 外す側を宣言に残す。
 *
 * 宣言した節に数の主張が **現れたら落ちる** = 数を書き戻した日に照合へ戻す。
 */
const 数を書かない節 = [
  {
    節: 節.カタログ,
    文: /分類の札を (\d+) 枚/u,
    理由:
      "枚数は `CATEGORIES` が決めるので、 仕様書は数ではなく出どころを書く (#1788)。" +
      " 表の中身の照合は `docs-derived-values.test.ts` が担う",
  },
] as const;
...
test("数を書かない節が、数を書き戻したら知らせる (宣言が古くなる形)", () => {
  expect(数を書かない節.length, "宣言が 1 件も無い (検査が空振りしている)").toBeGreaterThan(0);
  const 理由なし = 数を書かない節.filter((x) => x.理由.trim().length < 10).map((x) => x.節);
  expect(理由なし, `理由を書いていない宣言: ${理由なし.join(", ")}`).toEqual([]);

  const 書き戻した = 数を書かない節
    .filter((x) => x.文.test(節の本文(x.節)))
    .map((x) => x.節);
  expect(書き戻した, `宣言が古い (数を書き戻した): ${書き戻した.join(", ")}`).toEqual([]);
});

test("仕様書の中の数の主張と表の行数が一致する", () => {
  expect(数の主張.length, "数を見る節が 1 つも無い (検査が空振りしている)").toBeGreaterThan(1);
  for (const x of 数の主張) {
    const 本文 = 節の本文(x.節);
