...
const EXPECTED: Array<{ slug: string; id: string; rows: number }> = [
  ...
  /*
   * 箱の項目名の見本 (#2332、#2344 で 1 行にまとめた形を足した)。 英語 / 日本語 (段を分けた形) /
   * 日本語 (1 行にまとめた形) を切替で並べており、どれも `行` / `rows` を書いた表の箱を
   * 1 つ持つ (2 行)。
   */
  { slug: "text-dsl", id: "箱の項目名を1行にまとめて書く", rows: 2 },
  { slug: "text-dsl", id: "箱の項目名を英語で書く", rows: 2 },
  { slug: "text-dsl", id: "箱の項目名を日本語で書く", rows: 2 },
  /*
   * 行と印の見本 (#2377)。 印は ER 図と状態遷移図でだけ行頭の記号になるので、
   * 箱の項目名の見本 (フロー) から分けた。 英語 / 日本語を切替で並べ、どちらも
   * 3 行の表と 2 行の表を 1 つずつ持つ。
   */
