 ...
 * 見る窓の長さ (ms)。
 *
 * **2 段目に届く長さを取る**。 折れ線の見本は 2 段とも 2.4 秒 (#1440 で 2 段目を 1 段目に
 * 揃えた)。 0.5 倍速では 1 段目が 4.8 秒になるので、窓の頭が 1 段目の先頭に当たった時でも
 * 2 段目へ入れる長さが要る。
 *
...
test.describe("切替が拡大表示にも届く (#1361)", () => {
  test("既定のまま拡大すると、段の長さは見本のまま (陰性対照)", async ({ page }) => {
    ...
    // 見本の値 = 2 段とも 2400ms (#1440)
    expect([...長さ].sort((a, b) => a - b)).toEqual([2400]);
  });
  ...
  test("2x にしてから拡大すると、段の長さが半分で出る", async ({ page }) => {
    ...
    const { 値: 長さ, 訳 } = await 拡大の段の長さ(page);
    expect(長さ.size, `拡大表示の札を読めていない (${読めない訳(訳)})`).toBeGreaterThan(0);
    expect([...長さ].sort((a, b) => a - b)).toEqual([1200]);
  });
