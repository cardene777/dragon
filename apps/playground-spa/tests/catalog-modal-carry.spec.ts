 ...
 * 見る窓の長さ (ms)。
 *
 * **1 周より長く取る**。 折れ線の見本は 1 段目 2.4 秒 + 2 段目 0.9 秒で (#1444 で 2 段目を
 * 既定へ戻した)、0.5 倍速では 1 周 6.6 秒になる。 窓の頭が 1 段目の先頭に当たった時でも
 * 2 段目へ入れる長さが要る。
 *
 * **窓を 2 つ使う test がある**。 速度と描き方を同時に見る test は段の長さと線の引き直しを
 * 続けて観測するため、窓 2 つ分 + 開くまでの 2 秒強が test の持ち時間 (30 秒) に収まる必要が
 * ある。 9 秒なら 2 つで 18 秒、余りは 10 秒ある。
 *
 * **回数ではなく時刻で区切る** (#1438)。 回数で区切ると、1 回あたりの待ちが伸びた分だけ
 * 窓が伸びて持ち時間を超え、理由が出ないまま時間切れになる。
 */
const 見る窓 = 9_000;
 ...
 * 拡大表示の札に出た段の長さを集める。
 *
 * 図は繰り返し再生されるので、窓の間ずっと見て出た値を全部拾う。 2 段とも同じ長さなので
 * 落ち着いていれば 1 種類しか出ない。
 *
 * **1 つ見つけた時点で切り上げない** (#1440)。 窓の最後まで見ることで「他の数が出ない」
 * ことまで確かめられる = 段の長さが片方だけずれた時に、余った数が出て落ちる。
 */
...
test.describe("切替が拡大表示にも届く (#1361)", () => {
  test("既定のまま拡大すると、段の長さは見本のまま (陰性対照)", async ({ page }) => {
    ...
    // 見本の値 = 1 段目 2400ms / 2 段目 900ms。 揃うのは描き直す時だけ (#1444)
  ...
  test("速度と描き方を同時に変えても、両方が拡大表示に届く", async ({ page }) => {
    ...
    // **1 種類しか出ない**。 描き直すを選ぶと 2 段目の長さが 1 段目に揃うため (#1444)。
    // 同じ 0.5x でも動かすだけなら 2 種類 (1800 と 4800) 出るので、
    // ここが 1 種類であること自体が「揃える処理が効いている」 ことの証拠になる
    expect([...長さ].sort((a, b) => a - b)).toEqual([4800]);
