 ...
 *
 * 1440px の値は `#1100` で更新した。 読める下限 (`#1084`) を入れた後、 絵の枠 688px では
 * 見本 12 件のうち 7 件で登場人物が画面の外に出ていたため、 割り当てを
 * `260px 1fr 1.4fr` → `260px 1fr 2.6fr` に変えた (記法欄 492 → 340 / 絵 688 → 840、 記法欄には床 340px を置いた)。
 * 画面外がある見本は 7 件 → 3 件になった。
 *
 * 1024px と 900px は変えていない (別の割り当てが効く幅)。 */
const 広い画面 = [
  { 幅: 1440, 脇: 260, 記法欄: 340, 絵の欄: 840 },
