 ...
 * 送ってから測るとほとんどの分類で図が出る (`parts` だけは送っても出ない)。
 */
...
const 宣言: ReadonlyMap<string, string> = new Map([
  ...
  // 分類の画面 9 件が測っているのは **並べて見る側** の台。
  // #2286 で「幅に合わせたままにし、読むのは拡大表示で行う」 と決めた = 直す対象にしない。
  // 読む先の拡大表示は #2284 で 10px に乗せ、そこへの入口が指で押せることを
  // `catalog-modal-readable.spec.ts` が測る。 行き先を持つので、この 8 件は残り続ける
  ...([
    ...
    // #2424 でフローの既定の向きを横にしたため、2 人のフローが 2 縦列に広がって
    // viewBox が 1052 まで伸びた。 それまでは縦に積んでいたので下限を割っていない
    [
      "primitives",
      "1052x206",
      20,
      5.4,
      " 横に広がったのは #2424 でフローの既定の向きを横にしたため",
    ],
    ["cookbook", "1133x552", 24, 5.98, ""],
  ] as const).map(([分類, viewBox, 指定, 実寸, 補足]) => {
    const 下限の倍率 = Math.min(10 / 指定, 1);
    // 下限で描いた図の幅と、台 (283px) に見える割合。 **書かずに導く** = 手で書くと
    // 図の形が変わった日にずれる
    const 下限での幅 = Math.round(Number(viewBox.split("x")[0]) * 下限の倍率);
    const 見える割合 = Math.round((283 / 下限での幅) * 100);
    return [
      `catalog/${分類}`,
      `図 (viewBox ${viewBox}) が幅 283px の台に収まるだけ縮み、指定 ${指定} の文字が ${実寸}px になる。` +
        ` 下限の倍率 ${下限の倍率.toFixed(3)} で描けば 10.0px に乗るが、幅が ${下限での幅}px になり` +
        ` 台には ${見える割合}% しか見えない = 一覧を眺める面が横に辿らないと判らない面になる。` +
        ` #2286 で幅に合わせたままにすると決めた。 読む先は拡大表示 (#2284 で 10.0px に乗せた)` +
        補足,
    ] as const;
  }),
]);
 ...
 * 送ると分類の画面から図が出て、下限を割る画面が現れる (いちばん小さいのは `ethereum` の 1.72px)。
 * どの画面が割っているかは下の `宣言` が持つ = ここに数を書くと直すたびに古くなる。
 *
...
/** 測り直す回数と間隔。 ガントチャートは 2.1 秒で静止し、1 周は 7 秒 (#2279 の実測) */
const 測り直す回数 = 4;
const 測り直す間隔 = 700;
 ...
 * 静止した姿へ寄せる (#2279)。
 *
 * **描き出しの動きは文字を 0 倍から 1 倍へ動かす**。 ガントチャートの帯の中の名前は入れ子の拡大率が
