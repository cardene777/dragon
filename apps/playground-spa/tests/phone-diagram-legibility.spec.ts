 ...
 * 直ったかどうかも判らないまま残る。
 *
 * 宣言した経路は、下限に届かない図がまだ残っていることを確かめる = 直ったら落ちるので、
 * 直した PR がこの行を外すまで気付ける。
 */
const 宣言: ReadonlyMap<string, string> = new Map([
  [
    "/",
    "図を器に収めるだけで、読める下限を課していない (実測 2.0-5.6px)。" +
      " 下限を課す仕掛け (`src/lib/readable-scale.ts`) は在るが、繋いであるのは編集画面だけ。 #2269 で直す",
  ],
  [
    "/preset/:id",
    "同じ理由で 19 件とも下限を割る (実測 2.0-8.1px)。" +
      " 拡大と移動の操作は在るので、読む人が拡げれば読めるが、開いた時点では 1 文字も読めない。 #2269 で直す",
  ],
]);
...
test("経路と見本を実装から読めている (空振り検知)", () => {
  ...
  for (const [経路, 理由] of 宣言) {
    expect(
      [...図を出す経路],
      `図を出さない経路を宣言しても意味が無い: ${経路}`,
    ).toContain(経路);
    expect(理由.trim().length, `宣言の理由が空: ${経路}`).toBeGreaterThan(0);
  }
});
...
for (const 経路 of 対象の経路) {
  ...
  test(`幅 ${携帯の幅}px の ${経路} の図の文字が読める大きさに届く`, async ({ page }, info) => {
    ...
    for (const path of 開く先) {
      ...
      for (const 図 of await 測る(page)) {
        図の数++;
        文字の数 += 図.文字数;
        全体の最小 = Math.min(全体の最小, 図.最小);
        if (図.最小 < 下限) {
          割った.push(`${path || "トップ"} ${図.最小.toFixed(1)}px "${図.最小の字}"`);
        }
      }
    }
    ...
    if (理由 !== undefined) {
      expect(
        割った.length,
        `宣言が古い。 ${経路} は下限 ${下限}px を割らなくなったので宣言を外す (${母数})\n理由に書いてあったこと: ${理由}`,
      ).toBeGreaterThan(0);
      return;
    }
    expect(
