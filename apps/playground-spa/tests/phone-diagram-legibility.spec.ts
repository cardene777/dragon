...
const 宣言: ReadonlyMap<string, string> = new Map([
  ...
  [
    "preset/funnel",
    "静止した姿でも 7.86px (`↓ 86.6%` の指定 11 × 倍率 0.714)。" +
      " 下限を決める母数が開いた時の文字だけで決まり、後から増える文字を見ていない。" +
      " 開いた直後は指定 14 の 4 件しか無く、下限が 10/14 = 0.714 で止まる。" +
      " 測り直しても上がらないので、動きの途中を見ているのではない (#2279 で分けた)",
  ],
]);
