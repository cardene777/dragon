...
const 宣言: ReadonlyMap<string, string> = new Map([
  ...
  // 分類の画面 8 件が測っているのは **並べて見る側** の台。 読める下限を課す仕掛け
  // (`readableScaleForWidth`) を繋げば 10.0px に乗ることは実測したが、繋ぐと並べて見る側の
  // 既定が「台の幅に合わせる」 から「台より広く描いて辿る」 に変わり、既定を前提に書かれた
  // 検査 5 本が落ちる。 既定をどうするかは画面の性格の判断なので #2286 で決める。
  // 押して開く拡大表示の側は #2284 で下限を課した (`catalog-modal-readable.spec.ts` が測る)
  ...([
    ...
    ([分類, viewBox, 指定, 実寸]) =>
      [
        ...
        `図 (viewBox ${viewBox}) が幅 283px の台に収まるだけ縮み、指定 ${指定} の文字が ${実寸}px になる。` +
          ` 並べて見る側は幅に合わせるのが既定で、読める下限を繋いでいない。` +
          ` 繋げば下限の倍率 ${(10 / 指定).toFixed(3)} で 10.0px に乗るが、既定が変わる (#2286 で決める)`,
      ] as const,
  ),
]);
