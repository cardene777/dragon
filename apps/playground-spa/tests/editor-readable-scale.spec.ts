 ...
 * 決めるため、 縦の空白を残したまま極端に縮む (実測 = 見本「利用者登録」 が 23%、 絵の枠の
 ...
 */
import { MIN_PX, 下限, 譲っても収まらない見本 } from "./readable-floor";
 ...
 * 1 つずつ踏む。
 *
 * 窓 1440×900 (絵の枠 840px) での実測 (2026-09-21)。
 *
 * | 見本 | 世界座標の最小文字 | 実際の倍率 | 画面上の最小文字 | 何を踏むか |
 * |---|---|---|---|---|
 * | swimlane | 20 | 40.6% | 8.1px | 下限が倍率を決める。 かつ **譲る側** (#1102) |
 * | flow | 22 | 45.5% | 10.0px | 10px でも 8px でも箱が枠から出る = **譲っても収まらない側** (#2432) |
 * | pie | 12 | 97.6% | 11.7px | 100% の頭打ちのすぐ手前 |
 * | class | 22 | 38.2% | 8.4px | 10px では箱が枠から出るため **譲る側** (#1320) |
 * | topology | 17.1 | 70.9% | 12.1px | 下限が効かない (収める倍率が 10px の下限 58.6% より上) |
 *
 * `pie` を落とすと上限を下げる変異 (100% → 80%) を見逃す (実測で確認)。 swimlane と flow と
 * class の下限は 40% 前後で、 どれも 80% を下回るため上限に触れない。
 *
 * `#1102` で swimlane は譲る側に落ちた (10px では箱が枠から出るため 8px まで下げる)。
 *
 * `#2432` で `topology` を足した。 それまで「下限が効かない」 側を踏んでいたのは `flow` で、
 * #2424 で横に長くなって譲っても収まらない側へ移ったため、踏む相手が居なくなっていた。
 *
 * ## 層 2 = 描画側の出力 (全見本を 1 件で回す)
 *
 ...
 *
 * | 見本 | 世界座標の最小文字 | `#1084` 前 | `#1084` 後 | `#1102` 後 | `#2432` 後 |
 * |---|---|---|---|---|---|
 * | swimlane | 20 | 4.6px | 10.0px | 8.0px (譲る) | 8.1px |
 * | er | 22 | — | 10.0px | 8.0px (譲る) | 8.0px |
 * | state-machine | 20 | — | 10.0px | 8.0px (譲る) | 8.0px |
 * | sequence | 20 | 7.1px | 10.0px | 10.0px | 18.2px |
 * | gantt / pie | 11 | 8.0 / 8.6px | 10.0px | 10.0 / 10.5px | 11.7px (pie) |
 * | c4 / topology | 17 | 9.5 / 10.2px | 10.0 / 10.2px | 11.6 / 10.2px | 12.1px (topology) |
 * | mind | 24 | 9.9px | 10.0px | 12.0px | — |
 * | flow | 22 | 13.3px | 13.3px (下限が効かない) | 13.3px | **10.0px (譲っても収まらない)** |
 *
 * `flow` だけ #2424 で向きが変わって値が動いた。 縦に積んでいた間は枠に余裕があり
 * 下限が効かなかったが、横に並べてからは 10px の下限が倍率を決めている。
 */
const SAMPLES = ["swimlane", "flow", "pie", "class", "topology"];
...
async function 測る(page: import("@playwright/test").Page) {
  return page.evaluate(() => {
    ...
    const 枠 = host?.getBoundingClientRect();
    // **いま枠から出ているかを、そのまま測る** (#2432)。 倍率を掛け直して見積もると
    // 図の左の余白が落ちて、出ているのに収まると読む (`flow` の箱の幅は 8px の倍率で
    // 839px になり 枠 840px に入るように見えるが、右端は外に残る)
    const 枠から出ている =
      枠 === undefined || 箱.length === 0
        ? false
        : Math.max(...箱.map((b) => b.right)) > 枠.right + 1 ||
          Math.min(...箱.map((b) => b.left)) < 枠.left - 1;
    return {
      最小: rows[0],
      件数: rows.length,
...
for (const slug of SAMPLES) {
  test(`見本 ${slug} を開くと文字が ${下限(slug)}px 以上で出る (#1084)`, async ({ page }) => {
    ...
    // **譲る側に入れてよいかを、譲る理由そのもので見る** (#1320 / #2432)。
    //
    // 譲る側へ余分に足した形は、その見本が実際に 10px を下回って描かれているかで落ちる
    // (`pie` を足すと 11.7px なので落ちる)。 譲ること自体は検査の側では起きないので、
    // 一覧に足しただけで満たされることはない。
    //
    // **見本は 3 通りに分かれる** (#2432)。 一覧がどれに入れているかと、画面の実際を照らす。
    //
    // | 状態 | 画面上の最小文字 | 箱 |
    // |---|---|---|
    // | 枠に収まる | 10px 以上 | 枠の中 |
    // | 譲れば収まる (`譲る見本`) | 8px 台 | 枠の中 |
    // | 譲っても収まらない (`譲っても収まらない見本`) | 10px のまま | 枠から出る |
    //
    // **判定に倍率の掛け直しを使わない**。 以前は「10px にした時の箱の幅」 を見積もって
    // 枠と比べていたが、図の左の余白が落ちるため 3 つ目を 2 つ目と取り違える
    // (`flow` の箱の幅は 8px の倍率で 839px になり 枠 840px に入るように見える)。
    // 画面に出ている事実 (文字の大きさ / 箱が枠から出ているか) だけで 3 通りを分ける。
    expect(m.箱数, "箱を 1 つも測れていない (検査が空振りしている)").toBeGreaterThan(0);
    if (譲っても収まらない見本.includes(slug)) {
      expect(
        m.最小.px,
        `譲っても収まらない見本なのに ${MIN_PX}px を下回っている (譲る側に落ちている疑い)`,
      ).toBeGreaterThanOrEqual(MIN_PX);
      expect(
        m.枠から出ている,
        `譲っても収まらない見本に入れているが、箱は枠に収まっている (箱幅 ${m.箱幅} vs 枠 ${m.枠幅})`,
      ).toBe(true);
    } else if (下限(slug) < MIN_PX) {
      // 譲った結果が実際に効いているかを見る。 描画側が譲る下限を無視して 10px まで
      // 拡げた形を捕まえる
      expect(
        m.最小.px,
        `譲る側なのに通常下限 ${MIN_PX}px のまま描かれている`,
      ).toBeLessThan(MIN_PX);
      // 譲る必要が無いのに足した形は、ここが捕まえる = 譲って収まった以上 箱は枠の中に
      // ある。 3 つ目と分けるのは「文字が 10px を下回っているか」 のほう
      expect(
        m.枠から出ている,
        `譲る側に入れているが箱が枠から出ている (譲っても収まらない見本 の側ではないか)`,
      ).toBe(false);
    } else {
      expect(
        m.枠から出ている,
        `どちらの一覧にも無いのに箱が枠から出ている (箱幅 ${m.箱幅} vs 枠 ${m.枠幅})`,
      ).toBe(false);
    }
  });
}
...
test("画面に出ていない文字は下限を決めない (#1084)", async ({ page }) => {
  // 見本「利用者・投稿・コメントのスキーマ」 には `display: none` の 20 の文字があり、 数えると
...
test("枠に余裕がある図では倍率を上げない (#1084)", async ({ page }) => {
  ...
  // 縦長で元から読める見本は、 下限を入れる前と同じ倍率のままになる。
  //
  // **見本を `flow` から `topology` に替えた** (#2432)。 #2424 でフローの既定の向きが
  // 横になり、`flow` は横に長くなって枠に収まらなくなった (箱の右端が枠の外)。
  // 「枠に余裕がある図」 という前提を満たさないので、この検査の見本には使えない。
  //
  // `topology` を選んだのは **既定の向きが縦のままの唯一の図種** だから
  // (`packages/dragon/src/compile/direction.ts` の `既定の向き`)。
  // 向きの既定をまた触ってもこの見本の性質は動かない。
  await page.setViewportSize({ width: 1440, height: 900 });
  await page.goto("editor#preset=topology");
  await page.waitForLoadState("networkidle");
  await page.waitForTimeout(2500);
  await 見本が開けたことを確かめる(page, "topology");

  const m = await 測れた(page);
  // 空振り防止 = 下限が効いていないことを先に確かめる。 収める倍率 (実測 70.9%) が
  // 10px の下限 (実測 58.6%) より上にあるので、下限は倍率を決めていない
  expect(
    m.最小.px,
    `下限が押し上げている (枠に余裕がある図ではない): ${m.最小.px}px`,
  ).toBeGreaterThan(MIN_PX);
  expect(m.倍率, `倍率が動いた: ${m.倍率}`).toBeGreaterThan(0.55);
  // 上限は 100% (`readable-floor.ts` の頭打ち) より下に置く。 下限が押し上げた時はここを超える
  expect(m.倍率, `倍率が上がった: ${m.倍率}`).toBeLessThan(0.95);
});
...
const 見本: ReadonlyArray<readonly [string, number]> = [
  ...
  ["solidity", 6], ["bar", 12], ["line", 15], ["funnel", 11],
  ...
  /*
   * 描画側が変わって 2 件減った (#1709 で実測)。 見本の記法は 1 文字も触っていない。
   *
   * | 見本 | 変更前 | 変更後 | 何が減ったか |
   * |---|---|---|---|
   * | `bar` | 13 | 12 | 横軸の目盛が 5 本から 4 本になった (`0 / 200 / 400 / 600`)。 書いた言葉は 8 件とも出ている |
   * | `stat` | 3 | 2 | 書いた `previous` (`"38"`) が描かれなかった |
   *
   * **`bar` は目盛の刻み方が変わっただけ**で、書いた言葉は減っていない。
   *
   * `stat` は [#1711](https://github.com/cardene777/dragon/issues/1711) で描画側を直し、
   * 前の値が `前 38` として出るようになったので 3 に戻した (実測)。
   */
  /*
   * 傾きの図 (#1709)。 見本に足された後、この一覧に並べていなかったため 1 度も
   * 確かめられていなかった (`editor-sample-coverage.spec.ts` が落ちて知らせていた)。
   *
   * 実測 = 見える文字 14 / 最小 11.7px / 大きさを読めない文字 0。
   */
  ["slope", 14],
  /*
   * 分かれ道の図 (#2535 で足した)。 #2513 で見本に並んだ後、この一覧に並べていなかったため
   * 1 度も確かめられていなかった (`editor-sample-coverage.spec.ts` が落ちて知らせていた)。
   *
   * 実測 (2026-09-25、窓 1440×900) = 見える文字 9 / 最小 12.0px / 大きさを読めない文字 0 /
   * 箱 4 つとも枠の中。 好ましい下限 10px を満たすので、`readable-floor.ts` の
   * 譲る側にも譲っても収まらない側にも入れない。
   */
  ["flowchart", 9],
];
...
test(`全 ${見本.length} 見本で描画側の文字が下限の計算に載る (#1084)`, async ({ page }) => {
  ...
  for (const [slug, 見える下限] of 見本) {
     ...
     * 1 度読むだけだと、その瞬間に何段目かで数が変わる (実測 = フロー 5-7 / 構成図 9-11 /
