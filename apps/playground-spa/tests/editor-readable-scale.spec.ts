...
const 見本: ReadonlyArray<readonly [string, number]> = [
   ...
   */
  ["gauge", 8], ["radial", 8], ["stat", 2], ["waffle", 6], ["stacked", 17],
  /*
   ...
   * | `stat` | 3 | 2 | 書いた `previous` (`"38"`) が描かれない。 描画側の `ChartStatNode` は名前と値しか出さない |
   *
   * **`bar` は目盛の刻み方が変わっただけ**で、書いた言葉は減っていない。 `stat` は
   * 書いた値が絵に出ない状態なので [#1711](https://github.com/cardene777/dragon/issues/1711)
   * に切り出した (描画エンジンは本 Issue の対象外)。 記録を下げるのは実測に合わせる訂正で、
   * 行き先のある形にしてある。
   */
  /*
