 ...
 *
 * このページの見本のうち、記法を持つのは 30 件だけだった。 残りはコードのタブが
 * 押せず、「編集画面で開く」 も出なかった (件数は書かない = 見本を足すたびに動く。
 * 全件は `src/lib/catalog-items.ts` の `CATALOG_ITEMS.primitives` が持つ)。
 *
 * **先にあった 30 件も図とずれていた**。 縦列の幅と見出し、箱の上の小見出し、段の札が
 * 落ちていて、コードのタブに別の図になる記法が出ていた。 当時の全件を出し直した。
 *
