 ...
 *
 * 以前は実証が判定を自前で書き写していた。 本番のガントチャートの矢じりの判定を「常に通る」 形に
...
/** ガントチャートの依存の矢印の線 (`Z` で閉じない `path`) と矢じり (`Z` で閉じる `path`) */
...
/** ガントチャートの依存の矢じり (`Z` で閉じる 3 点の `path`) */
const ガントチャートの矢じりの隙間 = 軸を組む<{ 帯たち: 外枠[]; 矢じりたち: 測った要素[] }>({
  名前: "ガントチャートの矢じりの先が、着く先の帯の上辺に触れる",
  見本: "presetGantt",
  下限: 1,
  待つ: 矢印が出るまで待つ,
  測る: async (page) => ({
...
const ファネル図の段 = '[data-cdl-role="funnel-stage"]';

const ファネル図の幅 = 軸を組む<number[]>({
  名前: "ファネル図の段の幅が上から下へ減っていく",
  見本: "presetFunnel",
  下限: 2,
  待つ: 一拍待つ,
  測る: async (page) =>
    (await 要素を測る(page, ファネル図の段)).map((段) => 枠の外枠へ(段.行列, 段.局所の外枠).幅),
  母数: (幅たち) => 幅たち.length,
  判定: (幅たち) =>
    幅たち.flatMap((今, i) => {
      const 前 = 幅たち[i - 1];
  ...
  壊す: (page) => 変換を足す(page, ファネル図の段, "scale(3 1)", "最後"),
});
...
const マインドマップの根 = 軸を組む<根の位置>({
  名前: "マインドマップの根が図の中央から縦横とも 20% 以内にある",
  見本: "presetMindMap",
  下限: 1,
  待つ: 一拍待つ,
  /*
