...
/** 上と下の余白の差をどこまで許すか。 字の輪郭の丸めで 1-2 は動く */
const 許す差 = 5;
...
async function 余白を測る(page: import("@playwright/test").Page): Promise<実測[]> {
  return await page.evaluate(() => {
    ...
    for (const n of document.querySelectorAll("svg [data-cdl-node]")) {
      ...
      if (!n.querySelector("[data-cdl-role='node-label']")) continue;
...
test("字を持つ箱で上と下の余白が揃っている (#1496)", async ({ page }) => {
  ...
  for (let i = 0; i < 候補.length; i += 束) {
    ...
    const actors = group
      .map((k, j) => `  - A${i + j}: { kind: ${k}, eyebrow: "小見出し", subtitle: "説明の行" }`)
      .join("\n");
    const flow = group.map((_, j) => `  - A${i + j} -> 相手: ""`).join("\n");
    const 相手 = `  - 相手: { eyebrow: "小見出し", subtitle: "説明の行" }`;
  ...
  const 対象 = 測れた.filter((x) => x.絵 <= x.h + 2);
  const 絵が越える = 測れた.filter((x) => x.絵 > x.h + 2).map((x) => x.kind);

  const 内訳 = `候補 ${候補.length} / 字を持つ ${測れた.length} / 測った ${対象.length} / 絵が枠を越える ${絵が越える.length} (${絵が越える.join(", ") || "なし"})`;
