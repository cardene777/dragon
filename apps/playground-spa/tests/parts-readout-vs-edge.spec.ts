...
const 宣言: ReadonlyMap<string, string> = new Map([
  [
    "parts-funnel-3",
    "矢印 fn-1 が読み取り値 `60 / 100` を、fn-2 が `23 / 100` を割る (本検査の実測)。" +
      " 三段の漏斗は上から下へ絞る形そのものが図の中身なので横に並べられない (#2129 で記録済)。" +
      " 直すには記法の engine で読み取り値か矢印の取り付き位置を変える必要がある (#2200)",
  ],
  [
    "parts-batch-collector",
    "矢印 bt-in が読み取り値 `10 / 10` を割る (本検査の実測)。" +
      " 届く箱と溜まりを同じ縦列の上下に置くことが図の中身 (1 件ずつ入ると溜まるのが" +
      " 同じ場所で起きる)。 直すには記法の engine 側が要る (#2200)",
  ],
  [
    "parts-backpressure",
    "矢印 bp-out が読み取り値 `40 / 100` を割る (本検査の実測)。" +
      " 3 帯に並べ直すと戻る線が 2 帯をまたぎ、記法と組み立て API で板の高さが" +
      " 372 と 464 に割れる (#2199 で実測)。 直すには記法の engine 側が要る (#2200)",
  ],
  [
    "parts-split-box",
    "矢印 sp-tag が読み取り値 `24 / 24` を割る (本検査の実測)。" +
      " 1 つの入口から 2 つへ分けるのが図の中身で、縦に置くことに意味がある。" +
      " 直すには記法の engine 側が要る (#2200)",
  ],
]);
