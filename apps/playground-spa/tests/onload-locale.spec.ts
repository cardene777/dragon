 ...
 *
 * 走査の engine (`@cardenelabs/cdl`) は指摘の本文を日本語で返し、言語を選ぶ口を持たない。
 * engine は別 repo なので、画面の側が軸ごとの型で読み直して英語を組む (#2464、
 * `src/lib/audit-detail.ts`)。 読めない形が来た時は軸の呼び名へ落ちるため、
 * engine が本文の形を変えても日本語がそのまま出ることはない。
 *
 * 本文に出る引用 (`"受付"` 等) は図を書いた人の言葉なので訳さない。
 * 開いている見本が日本語で書かれていれば、その字は英語の画面にも出る。
 */
