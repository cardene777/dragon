...
export const tweenSimple = diagram("tween-simple", { topic: "tween: 数値線形補間" })
  ...
  .phase("p", { duration: 2500, title: "Counter を滑らかに進行 (0 → 100)", body: "phase 内で counter を 0 から 100 へ滑らかに変化。" }, (p: PhaseBuilder) => p.activate("a").tween("counter", 0, 100).badge("tween 中"))
  .build();
...
export const tweenChain = diagram("tween-chain", { topic: "tween: 連続 phase で累積" })
  ...
  .phase("p1", { duration: 2000, title: "初動 (0 → 10)", body: "1 phase 目の tween。" }, (p: PhaseBuilder) => p.activate("a").tween("n", 0, 10).badge("p1"))
  .phase("p2", { duration: 2000, title: "加速 (10 → 50)", body: "前 phase の終端値から続けて tween。" }, (p: PhaseBuilder) => p.activate("a").tween("n", 10, 50).badge("p2"))
  .phase("p3", { duration: 2000, title: "完了 (50 → 100)", body: "最終 phase で 100 まで。 hold で静止表示。" }, (p: PhaseBuilder) => p.activate("a").tween("n", 50, 100).badge("p3"))
  .build();
...
export const badgePerPhase = diagram("badge-per-phase", { topic: "badge: phase ごと切替" })
  ...
  .phase("p1", { duration: 1500, title: "準備中", body: "header に badge='preparing' を表示。" }, (p: PhaseBuilder) => p.activate("a").badge("preparing"))
  .phase("p2", { duration: 1500, title: "処理中", body: "header の badge を 'processing' に切替。" }, (p: PhaseBuilder) => p.activate("a").badge("processing"))
  .phase("p3", { duration: 1500, title: "完了", body: "最終 phase で badge='completed'、 step 完了示唆。" }, (p: PhaseBuilder) => p.activate("a").badge("completed"))
  .build();
...
export const mixedTweenSet = diagram("mixed-tween-set", { topic: "tween + set 併用" })
  ...
  .phase("p1", { duration: 2400, title: "読込開始 (状態 + 進捗を併走)", body: "tween で数値、 set で文字列を同時更新。 1 phase 内で複数 state を制御可能。" }, (p: PhaseBuilder) => p.activate("a").tween("amount", 0, 50).set("phase", "loading").badge("loading"))
  .phase("p2", { duration: 2400, title: "完了 (状態 + 進捗を仕上げ)", body: "次 phase で完了状態へ。" }, (p: PhaseBuilder) => p.activate("a").tween("amount", 50, 100).set("phase", "done").badge("done"))
  .build();
