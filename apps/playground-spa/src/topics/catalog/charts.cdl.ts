 ...
 *
 * ## 組立て API 版と同じ図にならないものが 2 つある
 *
 * 記法に足りない分がそのまま出た。 **記法の穴として残す** = 組立て API に戻すと、 また
 * 「エディタで開けない」 に戻る。
 *
 * | 見本 | 違い | 記法に足りないもの |
 * |---|---|---|
 * | `mindMap` | `mind-map` ではなく `card` を並べた図になる | 記法の `mind` は別実装で、 `mind-map` 種別を作らない |
 * | `ganttTimeline` | 各工程が 1 コマ幅になる (期間が無い) | 記法の `gantt` は開始しか読まず、 終了を書けない |
 *
 * どちらも記法側を直す話で、 `cardene777/dragon#1159` の段 2 で扱う。
 */
