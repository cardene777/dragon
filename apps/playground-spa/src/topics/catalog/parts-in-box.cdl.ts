 ...
 * ## 部品を基準にして箱を置く (#2039)
 *
 * 位置を座標ではなく「誰の」「どちら側に」「どれだけ離して」 で書く。 向きを 4 つとも見せるのは
 * 書き方の頁 (`styles.cdl.ts` の「位置を相対で書く」) で、ここは **部品を基準にする形** だけを見せる。
 *
 * **基準にする部品は座標で置く**。 基準にできるのは範囲が決まっている部品だけで、相対で書いた
 * 部品は組み立ての段階では座標が決まっておらず、範囲を出す側 (`partBoxes`) が「後で埋める」 と
 * して飛ばす。 範囲が決まるのは座標を書いた部品と、位置を 1 つも書かず格子に置かれた部品の 2 つ。
 *
 * このうち **格子に置いた部品を基準にすると縦位置が揃わない** (実測)。 格子の起点は本体の図の
 * 下端から決まるが、その本体には相対で置いた箱も入るため、基準を置く時点ではまだ下端が動く。
 * 同じ 3 つを格子で置くと 設備の稼働 が cy=1118、受付 が cy=654 と 464 離れ、座標で置くと 3 つとも
 * cy=400 で揃う。 効いたかを見る側 (`verifyPlacement`) も範囲を出す側と同じ値を見るため、
 * このずれに気付かず知らせを 1 件も出さない。 engine 側の直しは #2041 に切り出した。
 *
 * **基準にされる部品と、基準にして置く部品の両方を 1 枚に入れる**。 前者は部品の箱の範囲が、
 * 後者は部品の大きさが要る = 別の宣言が読む。 片方だけだと、もう片方が読まれていなくても絵が
 * 変わらない (実測 = 基準にされる側だけの図では、部品の大きさを出す宣言を空にしても記録が動かなかった)。
 *
 * **横に並べる**。 部品は普通の箱より深い (高さ 380-400 に対し 68) ため、部品の下に箱を置くと
 * 図が一覧の台に描ける高さ (900) を超える (実測)。 右に置けば高さは部品 1 つ分で収まる。
 *
 * **図の種類は `topology` で、縦列の囲みを外す**。 `flow` は登場人物を書いた順に矢印で繋ぐので、
 * 位置だけを見せたい図に矢印が出る。 `topology` は矢印を作らないが、既定で縦列に破線の囲みを
 * 引き、相対で置いた箱だけが囲みの外に出る (実測 = 囲みの中が 1 箱、外が 1 箱に割れた)。
...
export const subtitle__partInBox =
  "部品の名前を種類に書いて箱として置き、状態と倍率と色番号を書き換え、縦列に置き、部品の中の要素へ矢印を繋ぎ、2 つの部品へ矢印を分け、部品どうしを繋ぎ、高さの違う部品どうしも繋ぎ、2 つの部品から同じ箱へ集め、流れの途中に置き、名前を添えて並べ、部品を基準にして箱を置く";
...
export const sourceYaml__pattern__partInBox__部品を基準にする = `title: "設備の稼働の右に受付を置き、受付の右に乾燥炉の温度を置く"
type: topology
...
export const sourceJson__pattern__partInBox__部品を基準にする = `{
  "title": "設備の稼働の右に受付を置き、受付の右に乾燥炉の温度を置く",
  "type": "topology",
  "lanes": { "main": { "contain": false } },
  "actors": [
    { "name": "設備の稼働", "kind": "state-indicator", "posX": 400, "posY": 400 },
    {
      "name": "受付",
      "kind": "card",
...
export const pattern__partInBox__部品を基準にする = textDslToDiagram(
  sourceYaml__pattern__partInBox__部品を基準にする,
  { partsCatalog: 部品の一覧 },
);
