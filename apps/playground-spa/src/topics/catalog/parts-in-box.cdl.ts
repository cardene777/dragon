 ...
 *
 * **基準にする部品は位置を書かない**。 基準にできるのは範囲が決まっている部品だけで、相対で書いた
 * 部品は組み立ての段階では座標が決まっておらず、範囲を出す側 (`partBoxes`) が「後で埋める」 と
 * して飛ばす。 位置を書かない部品は格子に置かれるので範囲が決まり、それを基準にできる。
 *
 * #2039 で足した時は基準に座標 (`位置: 400,400`) を書いていた。 格子の起点が相対で置く箱の分だけ
 * 後から動き、基準の部品だけが下に落ちていたため (実測 = 設備の稼働 が cy=1118、受付 が cy=654 で
 * 464 離れた)。 #2041 で起点から相対で置く箱を外したので、座標を書かなくても 3 つとも cy=490 で
 * 揃う。 図も 1649 × 684 から 1489 × 774 に戻り、どちらの器でも境の内側に入る。
 *
