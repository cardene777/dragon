...
// ## `_h` は見えない支え (#2511)
//
// 読み取り値だけを持つ部品は、`_h` という 1×1 の見えない箱を 1 つ置く。
// 箱が 1 つも無いと帯の位置が決まらず、読み取り値だけが宙に浮く
// (割合の円で実測 = 外すと板が `89.5 / 68 / 270.5 × 121` から
// `-60 / -60 / 420 × 188` に変わる)。
//
// **描画側の点検はこの箱を「使われていない、書き違いかもしれない」 と知らせるが、
// 知らせのほうが誤り**。 従って消すと部品が壊れる。 知らせを出しているのは
// 読み取り専用の依存 (`cdl`) で、本 repo からは直せない。
//
// 外せないことは `packages/dragon/test/spacer-node-2511.test.ts` が
// `_h` を持つ部品を走査して 1 枚ずつ確かめる。
