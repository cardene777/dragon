...
export const FORMS: Section[] = [
  ...
  },
  {
    ...
    lines: [
      { code: "  - 時計: alarm-clock", note: "部品の名前を種類に書く" },
      { code: "  - 実績:", note: "変えられる値があれば縦に並ぶ" },
      { code: "      kind: achievement", note: "" },
      { code: "      倍率: 2", note: "部品だけを大きくする" },
  ...
  },
  {
    ...
    lines: [
      ...
      { code: "  - 画面: { lane: front }", note: "箱をその縦列へ入れる" },
      { code: "  - 受付: { lane: back }", note: "全ての箱に書く (一部だけだと知らせが出る)" },
      { code: "  - 記録: { lane: back }", note: "同じ縦列に書けば縦に積む" },
    ],
  },
  {
