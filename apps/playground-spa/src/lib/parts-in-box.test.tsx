...
describe("部品を箱に使う見本 (#1973)", () => {
  ...
  it("高さの違う部品どうしを繋ぐ切替は、縦にずれた相手へ角を裏返さずに繋ぐ (#2012)", async () => {
    /*
     * 高さの違う部品を横に繋ぐと、繋ぎ口の高さが 10 ずれる。 この 10 は角の丸み (14) の
     * 2 倍より短く、描画エンジンが丸みを頭打ちにするまでは角が行き過ぎて線が戻っていた
     * (`@cardenelabs/cdl` 0.65.0 / cdl#865)。
     *
     * ずれの大きさを先に見るのは、この見本が **見せたい形を通っていること** を確かめるため。
     * 同じ種類の部品どうしに書き換えるとずれが 0 になり、角の行き過ぎの検査は通ったまま
     * 見本だけが別の形になる。
     */
    const 並び = (await 見本()).patterns ?? [];
    const p = 並び.find((x) => x.名 === "高さの違う部品どうしを繋ぐ");
    expect(p, "高さの違う部品どうしを繋ぐ切替が無い").toBeDefined();
    const laid = layout(p!.diagram);
    // 要素の id の後ろ (`__ind` / `__mercury`) では引かない = 部品の種類を書き換えた時に
    // 「矢印が無い」 で落ち、高さのずれを見る下の検査に届かなくなる
    const e = laid.edges.find(
      (x) => x.from.startsWith("成形機__") && x.to.startsWith("乾燥炉の温度__"),
    );
    expect(e, "成形機から乾燥炉の温度への矢印が無い (検査が空振りしている)").toBeDefined();

    // `M ax ay L cx ay Q mx ay, mx y1 L mx y2 Q mx by, ex by L bx by` の数を順に読む。
    // 弧は制御点と終点で 4 つずつ持つので、合わせて 16 個
    const 数 = [...e!.d.matchAll(/-?[\d.]+/g)].map((m) => Number(m[0]));
    expect(
      数.length,
      `角を 2 つ持つ道筋になっていない (繋ぎ口の高さが揃うと直線になり、この見本が見せたい形を通らない): ${e!.d}`,
    ).toBe(16);
    const [, ay, , , , , , y1, , y2, , by] = 数;
    const ずれ = Math.abs(by! - ay!);
    expect(ずれ, `繋ぎ口の高さが揃っている (この見本が見せたい形を通っていない): ${e!.d}`)
      .toBeGreaterThan(0);
    expect(ずれ, `繋ぎ口の高さのずれが丸み 2 つ分 (28) 以上ある: ${e!.d}`).toBeLessThan(28);

    // 縦の走りが繋ぎ口と同じ向き (0 を含む) = 行き過ぎて戻っていない
  ...
  it("部品の一覧には部品そのものだけが入り、部品を箱に使う見本は入らない", async () => {
    ...
    expect(部品の数, "部品を 1 つも数えられていない (検査が空振りしている)").toBe(106);
    // 頭付きと頭なしの 2 通りで引ける
