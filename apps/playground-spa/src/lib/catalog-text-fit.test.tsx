 ...
 *
 * **`0xc1892f…` は 0.78.0 で消えた** (#2562 で足し、#2570 で外した)。 連鎖の箱
 * (`shape-blockchain-block`) の 3 行目の既定値が絵の幅に入らず切れていたもので、描く側が
 * 既定値を 12 字から 9 字へ縮めた
 * ([cardene777/cdl#885](https://github.com/cardene777/cdl/issues/885))。
 * 描く側の壊れをこちらの一覧で素通ししていた形で、外す判断は実物から取った
 * (版を上げた直後、この一覧を触る前に `描画側の literal は実物に存在する` が落ちた)。
 *
 ...
 * 足す時は、その `…` が記法を直しても消えないことを確かめる。 直せるものを足すと、
 * 描く側の壊れが素通しのまま残る (`0xc1892f…` がそうだった)。
 */
const 描画側の省略 = ["0x…"];
