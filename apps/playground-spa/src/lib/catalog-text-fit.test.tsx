 ...
 *
 * `0xc1892f…` は連鎖の箱 (`shape-blockchain-block`) の 3 行目に描く既定値
 * `0xc1892f4a...` が切れたもので、これも記法に由来しない (#2562)。 描く側が 0.73.0 で
 * 縮めた分を字の指定で打ち消すようになり ([cardene777/cdl#871](https://github.com/cardene777/cdl/issues/871))、
 * 字が 11 から 15.3 に上がって同じ幅に入らなくなった。
 *
 * **箱を広げても消えない**。 絵の幅は種別が持つ 240 で固定で、節点の `w` を変えても
 * 行の予算は動かない (実測 = `w` を 180 から 240 まで 7 通りで描いて、切れ方も字の大きさも
 * 1 つも変わらなかった)。 3 行目は副題の差し込み先でもないので、記法から値も変えられない。
 * 描く側の既定値を打ち消し後の大きさで収まる長さに縮める話として
 * [cardene777/cdl#885](https://github.com/cardene777/cdl/issues/885) に置いた。
 *
