 ...
 * ## 何を見るか
 *
 * カタログの全図を 1 度だけ描き、**その図が持たない種別の役割名が 1 つも出ていない** ことを見る。
 * 拾い方が図の種類を見ずに何でも拾っていたら、ここで落ちる。
 *
 * これは `catalog-axis-text` と `catalog-bar-look` が別々に持っていた陰性対照を 1 つに束ねた
 * もの。 2 つは棒と折れ線の役割名を手で並べていたが、ここでは **種別の一覧をカタログから導き、
 ...
 * ## なぜ 1 つに束ねたか
 *
 * 2 つともカタログのほぼ全図を描いており、**同じ図を 2 度描いていた**。
...
/** カタログに出ている図表の種別。 手で並べない = 種別が増えたら自動で対象になる */
...
describe("種別の印は、その種別を持つ図にしか出ない (#1704)", () => {
  ...
  it("カタログの図と図表の種別をどちらも 1 件以上走査できている", () => {
    // 空振り検知。 どちらかが 0 件だと下の検査は何も見ずに通る
    expect(図たち.length, "カタログの図が 1 件も無い").toBeGreaterThan(0);
  ...
  it("持たない種別の役割名が出ない", () => {
    ...
    // 母集団の内訳を出す。 総数だけ見ると、除外が増えて走査が痩せても気付けない
    const 総組 = 図たち.length * 種別.length;
    console.log(`[母集団] 図 × 種別 ${総組} 組 / 走査 ${測れた} / 除外 ${総組 - 測れた} (持っている組)`);
  });
