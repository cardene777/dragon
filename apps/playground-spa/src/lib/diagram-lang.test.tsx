...
describe("図に画面の言語を当てる", () => {
  ...
  it("カタログの倉庫の形は、日本語の画面で `倉庫` と描き、英語の画面で `WAREHOUSE` と描く", () => {
...
/** 図を描く入口を持つが、カタログの図を描かない入口。 理由を書く */
const カタログの図を描かない入口: Record<string, string> = {
  "../pages/RenderProbePage.tsx": "開発用の描画の調べ (`/__render`)。 自前で組んだ図を描き、カタログの図を描かない",
...
describe("画面が図を描く入口", () => {
  ...
  it("図を描く入口を持つ file を走査できている", () => {
    ...
    for (const file of Object.keys(カタログの図を描かない入口)) {
      expect(入口のfile.map(([f]) => f), `除外を宣言した ${file} に入口が無い`).toContain(file);
    }
  });

  it("カタログの図を描く入口は、すべて画面の言語を当てた図を渡している", () => {
    let 見た数 = 0;
    for (const [file, source] of 入口のfile) {
      if (file in カタログの図を描かない入口) continue;
    ...
    // カタログの図を描く入口は 一覧と拡大 (2) / 編集画面の図と部品の一覧 (2) / トップ (3) / プリセット詳細 (1) の 8 か所
