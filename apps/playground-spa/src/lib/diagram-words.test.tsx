...
/** 見本帳の図が持つ段の札。 型の形の札を図の型の一覧で引く検査が、母集団を実物から導くのに使う (#1872) */
...
describe("英語が片付いた見本の図の字 (#1778)", () => {
  it("対象の見本を見つけている", () => {
    ...
    // 宣言した図が見本帳に実在する (消えた図の宣言を残さない)。 **天井 0 の file に限らない** =
    // 天井の残る file の図も `英語の残り()` が同じ宣言を引くので、実在は見本帳の図全体で見る (#1916)
    expect(見本帳の図().件.map((r) => r.鍵)).toEqual(
...
describe("描画側の印は字の塊全体の時だけ外す (#1922)", () => {
  it("宣言した印が、どれかの図で字の塊として描かれている", () => {
    ...
    const 塊 = new Set(見本帳の図().件.flatMap((r) => 描いた字(r.図)));
