...
/**
 * カタログの選択肢と切り替えが見せる名前を持ち、名前を書かない形も切替で見比べられること (#1920)。
 *
 ...
 *
 * **名前を持つ見本はカタログから導く** = 手で並べると、組み立て関数にだけ名前を足した見本が
 * 記法の側で名前を落としたまま通る。
 */
...
describe("カタログの選択肢と切り替えの名前 (#1920)", () => {
  const 全部 = カタログの見本();
  ...
  it("名前を持つ見本は、操作部の選択肢と箱に差し込む初期値を名前で描く", () => {
    // カタログの名前つきの入力欄は `dropdown` と `toggle` だけ。 別の種類が名前を持ち始めたら落として検査を足す
    const 見た: string[] = [];
    for (const r of 名前つき) {
      const 字 = 描いた字(r.図);
  ...
  it("名前を書かない変種は、3 つの書き方とも名前を持たない", () => {
    for (const 鍵 of 名前を書かない変種) {
      ...
      expect(r, `${鍵} がカタログに無い`).toBeDefined();
