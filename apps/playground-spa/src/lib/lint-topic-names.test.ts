/**
 * 記法の検査の自動修正が書く図の説明が、見本帳の図の型の名前と同じ呼び名を使うことの検証 (#1934)。
 *
 * 自動修正 (`autoFix`) は、図の説明が型の名前 (`gantt` など) で始まる時に日本語の説明へ書き換える。
 * 書き換え先の表は記法の package (`packages/dragon/src/notation-lint.ts`) が持ち、見本帳の名前は
 * 画面側 (`ITEM_NAME_JA`) が持つ。 package は画面側の file を読まないので、2 つを照らす検査をここに置く。
 *
 * 直す前は `ガントチャート を示す図` のように、見本帳が #1930 で開いた名前 (`工程表`) と違う呼び名を
...
export const 見本の無い型: Record<string, string> = {
  chart:
    "型を決めない図表の総称。 見本帳は円グラフと折れ線グラフを別々の見本で置き、総称の見本は無い",
  "bar chart": "見本帳に棒グラフの見本が無い (図表の見本は円グラフと折れ線グラフの 2 つ)",
};
...
export function 名前とずれる型(
  ...
  for (const 型 of 型たち) {
    ...
    照らした++;
    const 説明 = 書き換える(型);
    if (!説明.endsWith(`を示す${名前}`)) ずれ.push(`${型}: ${説明} (見本帳は ${名前})`);
  }
  return { ずれ, 照らした };
}
...
describe("自動修正が書く図の説明と見本帳の名前 (#1934)", () => {
  ...
  it("植え込み対照: 見本帳の名前が変わると、書き換えた説明の末尾と重なる名前でもずれとして見つける", () => {
    const 変えた = { ...ITEM_NAME_JA, presetGantt: "ガントチャート", presetChartLine: "線グラフ" };
    expect(名前とずれる型(型たち, 書き換え, 変えた).ずれ).toEqual([
      "gantt: 作業の期間と前後の関係を示す工程表 (見本帳は ガントチャート)",
      "line chart: 値の移り変わりを示す折れ線グラフ (見本帳は 線グラフ)",
    ]);
  });
  ...
  it("植え込み対照: 名前の後ろに図を足す旧い文型は、名前を含んでもずれとして見つける", () => {
    ...
    expect(ずれ).toEqual([
      "gantt: 工程表 を示す図 (見本帳は 工程表)",
      "stateMachine: 状態遷移図 を示す図 (見本帳は 状態遷移図)",
    ]);
  });
