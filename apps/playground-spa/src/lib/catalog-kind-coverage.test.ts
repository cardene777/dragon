 ...
 * 分けるため。
 *
 * 以下 5 種は #1446 で依存を 0.15.0 に上げた時に増えた分で、**記法からまだ書けない**。
 * 図表 3 種は「前回の値」 や「時点」 を持つので名前と値だけの形では書き切れず、印 2 種は
 * 図表ではなく登場人物として書く形になるため、どちらも記法の形を先に決める必要がある。
 * 行き先は #1450。 **載せたままにすると検査が痩せる** ので、そこで空に戻す。
 */
const 対象外: ReadonlyArray<string> = [
  // 値 1 つを大きく示す。 「前回の値」 を書く形が未定 (#1450)
  "chart-stat",
  // 割合を 100 個の印で示す。 合計を 100 に揃える書き方が未定 (#1450)
  "chart-waffle",
  // 内訳と時点間の変化を帯で示す。 「時点」 を書く形が未定 (#1450)
  "chart-stacked-bar",
