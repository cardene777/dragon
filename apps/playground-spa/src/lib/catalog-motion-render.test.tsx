...
describe("図表の見本は数が段で動く (#1198)", () => {
  ...
  it("対象が 22 件ある", () => {
    expect(図表).toHaveLength(22);
  });

  it("22 件すべてが 2 段を持ち、段で動かす値を宣言している", () => {
  ...
  it("22 件すべてで、最初の段と最後の段で図の中の見える部分が変わる", () => {
    const 変わらない: string[] = [];
    for (const [k, d] of 図表) {
      const 最初 = 見える部分(d, d.phases[0]!.id);
      const 最後 = 見える部分(d, d.phases[d.phases.length - 1]!.id);
  ...
  it("22 件のうち数を動かす 18 件で、動かす値が絵の文字に出る", () => {
