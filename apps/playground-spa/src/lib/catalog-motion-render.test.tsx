...
describe("図表の見本は数が段で動く (#1198)", () => {
  ...
  it("14 件のうち数を動かす 12 件で、動かす値が絵の文字に出る", () => {
    ...
    expect(数で動く, "数を動かす図の数が変わっている").toHaveLength(12);
    const 出ない: string[] = [];
  ...
  it("14 件すべてで、項目が 1 つも落ちていない", () => {
    // 記法が `{名前}` を読めないと項目ごと落ちる。 落ちた図は「正しい図」 に見えてしまう。
    //
    // **一覧に無い図を黙って飛ばさない**。 期待値の無い図を素通りさせると、対象が増えた時に
    // その図だけ誰も数えないまま通る。
    //
    // 14 件とも「箱 1 つに配列を載せる型」 になった (#1177 / #1223 で `type: mind` を 1 箱の
