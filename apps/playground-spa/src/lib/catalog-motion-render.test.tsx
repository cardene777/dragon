...
describe("図表の見本は数が段で動く (#1198)", () => {
  ...
  it("対象が 17 件ある", () => {
    expect(図表).toHaveLength(17);
  });

  it("17 件すべてが 2 段を持ち、段で動かす値を宣言している", () => {
  ...
  it("17 件のうち数を動かす 14 件で、動かす値が絵の文字に出る", () => {
  ...
  it("17 件すべてで、項目が 1 つも落ちていない", () => {
    // 記法が `{名前}` を読めないと項目ごと落ちる。 落ちた図は「正しい図」 に見えてしまう。
    //
    // **一覧に無い図を黙って飛ばさない**。 期待値の無い図を素通りさせると、対象が増えた時に
    // その図だけ誰も数えないまま通る。
    //
    // 17 件とも「箱 1 つに配列を載せる型」 になった (#1177 / #1223 で `type: mind` を 1 箱の
