...
/**
 * 図の型の見本 (`presets` 19 件) は段ごとに絵が変わる (#1194 / #1204)。
 *
 * 動かし方は箱の数で 2 通りに分かれる。 箱を複数持つ 11 件は `scene-*` と同じく 1 つずつ
 * 光らせ、図全体が箱 1 つの 9 件は中身 (割合 / 段階 / 期間 / 感情 / 象限 / 名前 / 言づて) を
 * 状態から取る。
...
describe("図の型の見本は段ごとに絵が変わる (#1194)", () => {
  ...
  it("箱を複数持つ 10 件は、段ごとに光る箱が増える", () => {
    const 対象 = 型.filter(([, d]) => !箱が1つ.has(d.id));
    expect(対象, "箱が 1 つの型の一覧が実物とずれている").toHaveLength(10);
...
describe("図表の見本は数が段で動く (#1198)", () => {
  ...
  it("対象が 12 件ある", () => {
    expect(図表).toHaveLength(12);
  });

  it("12 件すべてが 2 段を持ち、段で動かす値を宣言している", () => {
  ...
  it("12 件すべてで、最初の段と最後の段で図の中の見える部分が変わる", () => {
    const 変わらない: string[] = [];
    for (const [k, d] of 図表) {
      const 最初 = 見える部分(d, d.phases[0]!.id);
      const 最後 = 見える部分(d, d.phases[d.phases.length - 1]!.id);
  ...
  it("12 件のうち数を動かす 10 件で、動かす値が絵の文字に出る", () => {
    ...
    // 語の欄で動く 2 件は位置と色が変わり、文字は変わらない。 数を動かす 7 件だけを見る
    const 語で動く = new Set(["初めて使うまで", "着手の順番"]);
    const 数で動く = 図表.filter(([, d]) => !語で動く.has(d.id));
    expect(数で動く, "数を動かす図の数が変わっている").toHaveLength(10);
    const 出ない: string[] = [];
  ...
  it("12 件すべてで、項目が 1 つも落ちていない", () => {
    // 記法が `{名前}` を読めないと項目ごと落ちる。 落ちた図は「正しい図」 に見えてしまう。
    //
    // **一覧に無い図を黙って飛ばさない**。 期待値の無い図を素通りさせると、対象が増えた時に
    // その図だけ誰も数えないまま通る。
    //
    // 12 件とも「箱 1 つに配列を載せる型」 になった (#1177 / #1223 で `type: mind` を 1 箱の
