...
describe("図表の見本は数が段で動く (#1198)", () => {
  ...
  it("対象が 18 件ある", () => {
    expect(図表).toHaveLength(18);
  });

  it("18 件すべてが 2 段を持ち、段で動かす値を宣言している", () => {
  ...
  it("18 件すべてで、最初の段と最後の段で図の中の見える部分が変わる", () => {
    const 変わらない: string[] = [];
    for (const [k, d] of 図表) {
      const 最初 = 見える部分(d, d.phases[0]!.id);
      const 最後 = 見える部分(d, d.phases[d.phases.length - 1]!.id);
  ...
  it("18 件のうち数を動かす 15 件で、動かす値が絵の文字に出る", () => {
    ...
    expect(数で動く, "数を動かす図の数が変わっている").toHaveLength(15);
    const 出ない: string[] = [];
  ...
  it("18 件すべてで、項目が 1 つも落ちていない", () => {
    // 記法が `{名前}` を読めないと項目ごと落ちる。 落ちた図は「正しい図」 に見えてしまう。
    //
    // **一覧に無い図を黙って飛ばさない**。 期待値の無い図を素通りさせると、対象が増えた時に
    // その図だけ誰も数えないまま通る。
    //
    // 18 件とも「箱 1 つに配列を載せる型」 になった (#1177 / #1223 で `type: mind` を 1 箱の
