...
describe("図の型の見本は段ごとに絵が変わる (#1194)", () => {
  ...
  const 箱が1つ = new Set([
    ...
    "seq-demo",
    // 複雑な版も同じ 1 枚の板 (#2161)。 面が 5 つでも箱は 1 つで、`seq_step` を進める
    "seq-complex-demo",
    // 図表は板 1 枚で描く (#2177)。 点が 12 でも箱は 1 つで、段は点の値を置き換える
    ...
    "gantt-complex-demo",
    // 体験の道筋も同じ (#2183)。 段階が 6 でも箱は 1 つで、段は気持ちの値を置き換える
    "journey-complex-demo",
    // 漏斗も同じ (#2185)。 段階が 7 でも箱は 1 つで、段は人数の値を置き換える
    "funnel-complex-demo",
    // 四象限図も同じ (#2187)。 項目が 12 でも箱は 1 つで、段は居る枠の値を置き換える
    "quad-complex-demo",
    // 階層図も同じ (#2189)。 箱が 12 でも図としては 1 つで、段は名前と人数の値を置き換える
    "tree-complex-demo",
    // 枝分かれ図も同じ (#2191)。 箱が 11 でも図としては 1 つで、段は 2 行目の値を置き換える
    "mind-complex-demo",
  ]);
  ...
  it("箱が 1 つの 18 件は、段が図表の中身を動かす", () => {
    // 箱が 1 つしか無いので光らせ方では動かせない。 値を動かす宣言を持つことを見る
    const 対象 = 型.filter(([, d]) => 箱が1つ.has(d.id));
    expect(対象).toHaveLength(18);
 ...
 *
 * 図表の見本は **記法で書く** (画面のコードのタブと「編集画面で開く」 を成立させるため)。
...
describe("図表の見本は数が段で動く (#1198)", () => {
  ...
  it("23 件すべてで、項目が 1 つも落ちていない", () => {
    ...
    const 期待: Record<string, { 中身: number }> = {
      経路別の流入: { 中身: 4 },
      // 棒を段の 4 割で伸ばし終える変種 (#1969)。 項目は元の図と同じ 4 つ
      "段の-4-割で伸ばし終える経路別の流入": { 中身: 4 },
      週ごとの応答時間: { 中身: 5 },
      費用の内訳: { 中身: 4 },
      申込みまでの絞り込み: { 中身: 4 },
      "図を速くする": { 中身: 4 },
