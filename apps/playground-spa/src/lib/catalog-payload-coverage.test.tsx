 ...
 * カタログには 1 件の見本しか無く、**割合が意味を持つ形がどこにも出ていなかった**。
 ...
 * 中身を持つ節は他に 7 つある (ガントチャート / 発想の枝 / 漏斗 / 四象限 / 系統樹 / ユーザージャーニー /
 ...
 * ## 両側は 1 つの見本の切替で見せる
 *
 * 「カタログのどこかに両側があればよい」 だと、別々の行に分かれていて **見比べられない**。
 ...
 *
 * 代わりに「カタログが 1 度は書いていること」 を求める。 カタログの役目は **記法で何が書けるか**
 ...
 *
 * 書いた欄が **絵に届いているか** も見る。 カタログの実物からその欄を消して描き比べ、絵が
 ...
 * 欄の名前を持つこと、`置き場所` は書いても役割が増えないこと、`動き` と `記法` はカタログが
...
/**
 * カタログが書けない欄。 **鍵は `節/<欄>` か `矢印/<欄>`、値はなぜ書けないか**。
 *
 * 記法 (`packages/dragon`) に口が無い欄は、カタログが書きようがない。 engine の型にだけ在って
 ...
 *
 * カタログが書いているのに、消しても絵が 1 px も変わらない欄がここに来る。 読み手には
 * 見えないので、見本を足しても意味が無い。
 *
...
const 届かない欄: Record<string, string> = {
  ...
  "矢印/cardinality":
    "関係の多重度。 記法から書いた値は組み立ての時に名前の下の行 (`sub`) と両端の形 (`tailHead` / `head`) へ写されるので、絵に出るのはそちらの側 (#2105)。 描画エンジンはこの欄を読まない",
};
...
const 覆えない組: Record<string, string> = {
  ...
  "journey-map/段/件数":
    "段が 1 つのユーザージャーニーには起伏が無い (`journey-line` が 1 本も出ない) = 点 1 つで、道筋にならない",
...
/**
 * どの種別も描いていない欄。 **鍵は `<節の欄>/<並び>/<軸>`、値はなぜ描かれないか**。
 *
 * 記法や型には在るのに、書いても絵が 1 px も変わらない欄がある。 見本を要求しても
 * 見せるものが無いので要求しない。
 *
 * **ここに要るのは、その家族の どの種別も 描かない欄だけ**。 一部の種別だけが描かない組は
 * 下の `この種別だけ描かない組` が持つ。 2 つの表に分けるのは意味が違うため =
 * 「誰も描かない」 は記法の側の穴で、「この種別だけ描かない」 は種別ごとの読み落とし。
 *
 ...
 * ## なぜ種別ごとに要るか
 *
 * #1707 は鍵に種別を含めなかった。 「種別ごとに見ると『棒グラフは前の時点を描かない』 の
 * ような当たり前の組が数十件並び、本当に誰も描かない欄が埋もれる」 という読みだった。
 *
 * **実測すると 10 件で、数十件にはならなかった**。 読みが外れていたので種別ごとに戻す。
 *
 * 含めなかった間、`previous` を読まない 5 種と `tone` を読まない 2 種が **どの検査からも
 * 見えていなかった** = その家族の別の種別 (円 / 内訳の帯 / 2 時点の線 / 大きな数字) が
 * 描いているため、家族単位では「描いている」 に数えられていた。
 *
 * 実際に 1 件が漏れた。 大きな数字が `previous` を読まない状態が
 * [#1711](https://github.com/cardene777/dragon/issues/1711) まで残り、見つけたのは
 * この検査ではなく **画面の文字数の記録が落ちたこと** だった。
 *
 * ## 理由は 2 つに書き分ける
 *
 * | 種類 | 意味 | 書き方 |
 * |---|---|---|
 * | 意味を持たない | その図の読み方では前の時点や色が置けない | なぜ置けないかを書く |
 * | 決めて読まない | 置けるが、置かないと決めた | 決めた Issue 番号を添える |
 * | 読み落とし | 置けるのに engine が読んでいない | 直す側の Issue 番号を添える |
 *
 * **「engine が読まない」 で止めない**。 止めると直すべきものと直さないものが同じ文になり、
 * 次に読む人が判断をやり直すことになる。
 *
 * **どれに当たるかは engine の実物を読んで決める**。 実測できるのは「絵が変わらない」 まで
 * で、なぜ変わらないかは判定に出ない。 棒の色は読み落としに見えたが、実物を読むと
 * 決めて読まない側だった (#1720)。
 */
 ...
 *
 * 書いた側はカタログの実物なので、値を作らずに済む = 欄ごとの見本値を手で並べると、
...
describe("中身を持つ節が見せる形をカタログが見せているか (#1694 / #1698 / #1700 / #1706)", () => {
  const 軸ら = 軸たちを作る();
  let 材料: 下ごしらえ;
  /** `<種別>/<並び>/<軸>` ごとの判定 */
  ...
  it("役割が増える組は、1 つの見本の切替で両側を見せている", () => {
    // 「カタログのどこかに両側がある」 では見比べられない。 同じ見本の パターン で持つ
    let 測れた = 0;
    const 足りない: string[] = [];
  ...
  it("両側が別の見本に分かれていると見つかる (植え込み対照)", () => {
     ...
     *
     * 「カタログのどこかに両側があればよい」 に緩めると、この対照が通ってしまう。
     */
  ...
  it("その種別だけ描かれない組は、表に理由が書いてある (#1718)", () => {
    /*
     * **家族単位の宣言では見えない**。 その家族の別の種別が描いていれば
     * 「描いている」 に数えられ、読んでいない種別が黙って通る。
     *
     * 実測 = `previous` を読まない 5 種と `tone` を読まない 2 種が、この検査を入れるまで
     * どの検査からも見えていなかった。
     */
    let 測れた = 0;
    const 書いていない: string[] = [];
    for (const [鍵, { 判定, 軸 }] of 判定表) {
      if (判定 !== "絵も変わらない") continue;
      測れた += 1;
      // 家族ごと描かない欄は そちらの表が持つ。 2 つの表に同じ組を書かない
      if (描かない欄[`${軸.欄}/${軸.並び.名}/${軸.名}`] !== undefined) continue;
      if (この種別だけ描かない組[鍵] === undefined) 書いていない.push(鍵);
    }
    expect(
      書いていない,
      `その種別が描かない組が表に無い: ${書いていない.join(", ")}`,
    ).toEqual([]);
    expect(測れた, "描かない組を 1 つも測れていない (検査が空振りしている)").toBeGreaterThan(0);
  });

  it("種別ごとの宣言は、今も engine が 1 px も描かない組だけ (#1718)", () => {
    // 描き始めたら見本が要る。 理由だけ残ると「書いてあるから」 で見逃す
    const 死んだ理由: string[] = [];
    for (const 鍵 of Object.keys(この種別だけ描かない組)) {
      const x = 判定表.get(鍵);
      if (x === undefined) {
        死んだ理由.push(`${鍵} (その組はもう無い)`);
        continue;
      }
      if (x.判定 !== "絵も変わらない") 死んだ理由.push(`${鍵} (engine が描くようになった)`);
    }
    expect(
      死んだ理由,
      `種別ごとの宣言に死んだ理由が残っている: ${死んだ理由.join(", ")}`,
    ).toEqual([]);
  });

  it("2 つの表が同じ組を二重に持たない (#1718)", () => {
    // 二重に持つと、片方を消しても もう片方が通してしまう
    const 二重: string[] = [];
    for (const [鍵, { 軸 }] of 判定表) {
      const 家族鍵 = `${軸.欄}/${軸.並び.名}/${軸.名}`;
      if (描かない欄[家族鍵] !== undefined && この種別だけ描かない組[鍵] !== undefined)
        二重.push(`${鍵} (家族 ${家族鍵} と両方)`);
    }
    expect(二重, `2 つの表が同じ組を持っている: ${二重.join(", ")}`).toEqual([]);
  });

  it("理由に行き先か、置けない訳が書いてある (#1718)", () => {
    /*
     * **「engine が読まない」 で止めない**。 止めると直すべきものと直さないものが同じ文に
     * なり、次に読む人が判断をやり直す。
     *
     * 読み落としには行き先 (Issue 番号)、意味を持たない組には なぜ置けないかを求める。
     */
    const 足りない: string[] = [];
    for (const [鍵, 理由] of Object.entries(この種別だけ描かない組)) {
      const 行き先あり = /`?(cdl|dragon)#\d+/.test(理由);
      // 行き先が無いなら、置けない訳が書いてあること。 20 字は「読まない」 の一言を弾く長さ
      if (!行き先あり && 理由.length < 20) 足りない.push(鍵);
    }
    expect(
      足りない,
      `理由が行き先も置けない訳も持たない: ${足りない.join(", ")}`,
    ).toEqual([]);
    expect(
      Object.keys(この種別だけ描かない組).length,
...
describe("節と矢印の任意の欄が、何かに覆われている (#1700 / #1707)", () => {
  ...
  for (const d of 図ら) {
    ...
    /*
     * **矢印の側は「書きぶり」 ごとに 1 図を選ぶ** = カタログの全図 (矢印を持つ 136 図) を
  ...
  it("記法と動きに割り当てた欄は、カタログが書いた図を持っている", () => {
    /*
     * カタログの役目は「記法で何が書けるか」 を見せること。 書ける欄を 1 度も書いていないと、
     * 読み手はその欄の存在を知りようがない。
     *
    ...
    expect(
      書いていない,
      `記法から書けるのにカタログが 1 度も書いていない欄\n  ${書いていない.join("\n  ")}`,
    ).toEqual([]);
    expect(測れた, "記法と動きに割り当てた欄が 1 つも無い (検査が空振りしている)").toBeGreaterThan(0);
  });

  it("記法に割り当てた欄は、消すと絵が変わる図がカタログにある", () => {
    /*
     * 書いてあるだけでは足りない。 **書いた欄が絵に届いていること** まで見る。
     *
     * 判定はカタログの実物から欄を消して描き比べる = 欄ごとの見本値を手で並べると、実物と
     * 違う値で測ることになる。
     *
