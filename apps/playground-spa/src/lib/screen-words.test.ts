 ...
 * 基準をまだ当てていない材料 (`カタカナの判定を待つ材料`、#1930) は **待っている間に増えても
 ...
 *
 * **#1929 で下がった** (字 390 → 354、語 236 → 232、延べ 453 → 411)。 見本帳の分類の説明と
 * 見本集の札を判定に入れ、開く語を開いた。 画面に出ていなかった分類の代表の中身 (`items`) も消した。
 */
const カタカナの天井 = { 字: 354, 語: 232, 延べ: 411 };
...
describe("画面の字に残るカタカナ語の歯止め (#1855)", () => {
  it("いまの天井を超えていない、下回ってもいない", () => {
    ...
    for (const [名, いま, 上] of [
      ...
      expect(
        いま,
        `${名} が天井を超えた。 増えた語を #1854 の基準で開く (判定を待つ材料の語は #1930 が片付ける) (${上} → ${いま})`,
