 ...
 *
 * **#2461 で上がった** (字 200 → 202、語 74 のまま、延べ 230 → 232)。 見本ごとの英語を持つ
 * 対訳表 (`catalog-item-en.ts`) を足し、変種の名前は日本語を鍵にして引く形にした。 増えた 2 回は
 * どちらも鍵の `フローで書く` で、画面に新しく出た語ではない (同じ名前を材料の側ともう 1 度
 * 数えている)。 **開く側では直せない** = 鍵は材料の側の名前と 1 字も違えられない。
 *
 * **#2513 で上がった** (字 202 → 203、語 74 のまま、延べ 232 → 234)。 記法で `flowchart` を
 * 書けるようにして、記法の頁に見本を 1 枚足した。 その日本語名 `テキスト記法のフローチャート` が
 * `テキスト` と `フローチャート` を 1 つずつ含むので、字は 1 つ、延べは 2 回増える。
 * 頭が `テキスト記法の…` に固定されるのは #2332 と同じ理由で、`フローチャート` は #2420 で
 * 分かれ道の図を指す呼び名として残すと決めた分。 **どちらも開く側では直せない**。
 *
 * **#2539 で下がった** (字 203 → 201、語 74 のまま、延べ 234 → 232)。 上の帯から
 * 「この画面を共有」 を外し、写せた時と写せなかった時の知らせ 2 つが消えた。
 * どちらも `コピー` を 1 回ずつ含む。 語の種類が減らないのは、`コピー` が編集画面の側に
 * まだ 10 回出るため。
 */
const カタカナの天井 = { 字: 211, 語: 78, 延べ: 244 };
