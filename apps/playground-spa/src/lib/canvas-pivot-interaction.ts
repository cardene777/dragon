 ...
 *
 * 2026-07-24 fix = parts merge lane (例 `achievement1__l`) に対応。
 * `__` 区切り alias が現れたら prefix 部分を actor 名として resolve、 subKey 部分は捨てて lane 全体経路にする。
 * これがないと achievement drop 後の parts lane 全域 click が pan mode に fallback して画面全体が pan する。
 */
function resolveDslName(rawId: string, slugToName: Map<string, string>): string | null {
  const stripped = rawId
    ...
    .replace(/-spacer$/, "")
    .replace(/^s\d+-/, "");
  const hit = slugToName.get(stripped);
  if (hit) return hit;
  // parts merge alias `{alias}__{subId}` (CAR-1657 unified syntax) の lane 経路対応。
  // node 経路 (resolveDslNameWithSubKey) は既に `__` を認識するが、 lane 経路は未対応で
  // pan mode fallback の root cause になっていた (decision-log 2026-07-24)。
  const usIdx = rawId.indexOf("__");
  if (usIdx > 0) {
    const aliasPart = rawId.slice(0, usIdx);
    const aliasHit = slugToName.get(aliasPart);
