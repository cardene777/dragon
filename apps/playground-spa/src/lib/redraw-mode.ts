 ...
 * 忘れると、新しく足した弧の図だけ古い初期値で出る。
 *
 * 切替を出せない図では `既定の描き方` を返す。 押しても何も変わらない図の初期値を動かすと、
 * 画面に出ない値が図ごとに違う状態になる。
 */
export function 図ごとの既定の描き方(diagram: CdlDiagram): 描き方 {
  if (!描き方を選べる(diagram)) return 既定の描き方;
  return diagram.nodes.some((n) => n.kind === 描き直しから始める種別) ? "描き直す" : 既定の描き方;
}
