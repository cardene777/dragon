...
/**
 * 起点から描き直した方が読める図の種別 (#1690)。
 *
 * 弧は「12 時からどこまで回ったか」 を読む図で、既にある弧の先へ継ぎ足すと起点を見失う。
 * 1 段目が 12 時から開く動きなので、2 段目だけ継ぎ足しになると揃わない。
 */
const 描き直しから始める種別 = "chart-radial";

/**
 * 描き直すに閉じる図か (#1690 / #1692)。
 *
 * 図の節の種別から導く。 **項目の id を並べた表は持たない** = 見本が増えた時に表の更新を
 * 忘れると、新しく足した弧の図だけ古い扱いで出る。
 *
 * 写す先が無い図 (段が 1 つ / 起点から描けない) は対象外。 写せないのに閉じると、
 * 画面から切替が消えたまま描き方も変わらない状態になる。
 */
function 描き直すに閉じる図(diagram: CdlDiagram): boolean {
  return 描き方を選べる(diagram) && diagram.nodes.some((n) => n.kind === 描き直しから始める種別);
}
 ...
 * 描き方の切替を画面に出すか (#1692)。
 *
 * 閉じる図では出さない = 弧で 動かすだけ を選ぶと、既にある弧の先へ継ぎ足す動きになり
 * 1 段目の「12 時から開く」 と揃わない。 カタログは「この記法でこう描ける」 を見せる場所で、
 * 読めない描き方を選べる状態にしておく理由がない。
 *
 * **`描き方を選べる` は false 側へ倒さない**。 あれは `図の描き方を変える` が写せるかを
 * 見る判定で、false にすると弧に `描き直す` を写せなくなり初期値ごと効かなくなる。
 * 画面に出すかと、変換が効くかは別の問い。
 */
export function 描き方の切替を出すか(diagram: CdlDiagram): boolean {
  return 描き方を選べる(diagram) && !描き直すに閉じる図(diagram);
}
