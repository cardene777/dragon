...
/**
 * 動きを表す語。 説明に出たら弾く (#1043)。
 *
 * 直す前は「連続した動きを語る言い回し」 だけを列挙し、当たった説明を実装と突き合わせていた。
 * 言い回しは無限にあり収束しない。 **今は動きの語を 1 つも書かせない** ため、
 * 連続か段階かを区別する必要が無く、列挙の役割が「主張の判定」 から「書き忘れの指摘」 に変わった。
 *
...
describe("手本の形 (#1033)", () => {
  ...
  it("一覧の説明が動きを語っていない", () => {
    // 動きの種類は **人が書かず図から導く** (#1043)。 説明に書くと実装とずれ、
    // ずれは言い回しの列挙では止められない (実測 = 「値がゆっくり変わる」
    // 「棒が次第に伸びる」 「じわじわ増える」 「シームレスに切り替わる」 「漸次変化する」
    // 「アニメーションしながら変わる」 の 6 つが、直す前の列挙をいずれも素通りした)。
    //
    // **この列挙は補助**。 取りこぼしても、導いた文が隣に出ているため
    // 「誤った主張が単独で出る」 ことは起きない。 余計な一文が並ぶだけになる
    const bad: string[] = [];
    for (const k of ALL) {
      const caption = (mod as unknown as Record<string, unknown>)[`subtitle__${k}`];
      if (typeof caption !== "string") continue;
      const found = MOTION_WORDS.filter((re) => re.test(caption)).map((re) => re.source);
      if (found.length) bad.push(`${k}: ${found.join(" / ")}`);
    }
    expect(
      bad,
      `説明が動きを語っている (動きは図から導くので書かない):\n${bad.join("\n")}`,
    ).toHaveLength(0);
  });

  // 動きの記述が実装から導かれていることは `catalog-motion.test.ts` が見る。
  // ここは説明文の側だけを受け持つ (説明が動きを語っていないこと)。
