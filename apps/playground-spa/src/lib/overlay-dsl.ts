 ...
 * @param baseBottom 既存の図の下端 (`partsBaseBottom`)。 段数の目安ではなく実測値 (#2002)
...
export function placeParts(
  ...
  const grid = partsGridCenters(baseBottom, autoIds);
...
export function 図と重ねる部品に分ける<R extends { diagram: CdlDiagram }>(
  ...
  if (parts.length === 0) return { built: 組み立てる(baseSrc), parts, 抜かずに描いた部品: [], lineMap };
