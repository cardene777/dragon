 ...
 * 死蔵として落とす。
 */
const 揃える呼び名: { 直す: string; 揃える先: string; 理由: string }[] = [
  {
    直す: "パーツ",
    揃える先: "部品",
...
/** 1 つの file から、画面に出る字 (引用符の中と地の文) をまとめて取る */
function 画面に出る字(path: string): string[] {
  const 字 = 見本を外す(コメントを外す(readFileSync(path, "utf8"))).字;
  return [...引用符の文言(字), ...地の文(字).文].filter((文) => 日本語の字.test(文));
}

/** その字が、揃えるべき呼び名を含んでいるか。 本番と対照が同じ関数を使う (#1811) */
export function 二通りの呼び名(文: string): string[] {
  return 揃える呼び名.filter((対) => 文.includes(対.直す)).map((対) => `${対.直す} → ${対.揃える先}`);
}
...
describe("画面の枠の文言 (#1795)", () => {
  ...
  it("同じものを 2 通りに呼んでいない (#1811)", () => {
    const files = 画面のfile一覧(/\.tsx?$/);
    expect(files.length, "画面 file を 1 つも見ていない (検査が空振りしている)").toBeGreaterThan(3);
    let 走査 = 0;
    const 残る: string[] = [];
    for (const f of files) {
      for (const 文 of 画面に出る字(f)) {
        走査 += 1;
        for (const 印 of 二通りの呼び名(文)) {
          残る.push(`${f.slice(置き場.length)}: ${文} [${印}]`);
        }
      }
    }
    console.log(`[呼び名] file=${files.length} 走査した字=${走査} 対=${揃える呼び名.length} 残る=${残る.length}`);
    expect(走査, "字を 1 つも読めていない (検査が空振りしている)").toBeGreaterThan(50);
    expect(残る, `同じものを 2 通りに呼んでいる:\n${残る.join("\n")}`).toEqual([]);
  });

  it("揃える先が実際に画面で使われている (#1811)", () => {
    // 揃え終わった語を一覧に残すと、行だけが増えて何も守らなくなる
    const files = 画面のfile一覧(/\.tsx?$/);
    const 全字 = files.flatMap((f) => 画面に出る字(f));
    const 出ない = 揃える呼び名
      .filter((対) => !全字.some((文) => 文.includes(対.揃える先)))
      .map((対) => `${対.直す} → ${対.揃える先}`);
    expect(出ない, `揃える先が画面に 1 度も出ていない: ${出ない.join(", ")}`).toEqual([]);
  });

  it("2 通りの呼び名を拾える (植え込み対照、#1811)", () => {
    // **植え込む字を表から作らない**。 表から作ると表を壊しても落ちない = 恒真になる
    // (実測 = `パーツ` を `ぱーつ` に書き換える変異が 0 件 FAIL だった)。
    // ここに書く字は、直す前にこの画面が実際に出していた字そのもの
