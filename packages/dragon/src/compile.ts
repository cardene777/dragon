 ...
 * (`type: flow` で `大きさ:` を書いて掃いた)。 **字を変えたら測り直す** = 下ばみの量が
 * 変わるので、 表だけ据え置くと箱が余るか名前がはみ出す (#1110 で字を変えた時に 4 種とも動いた)。 描画側の `y` と font の大きさから式で出さない
...
const LABEL_MIN_H: Readonly<Record<string, number>> = {
  actor: 94, // baseline y=86 / fontSize 32 (実測 = 94 で -0.66、 93 で +0.55)
  function: 92, // y=86 / 24 mono (92 で -0.24、 91 で +0.96)
  storage: 83, // y=78 / 22 mono (83 で -0.04、 82 で +1.17)
  event: 95, // y=88 / 26 mono、 0.92 倍に縮めて描かれる (95 で -0.87、 94 で +0.12)
