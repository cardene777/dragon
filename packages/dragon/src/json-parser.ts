...
export interface JsonActor {
  ...
  /**
   * CAR-1657 unified syntax = 既存 NodeKind (一覧は `v05/parser.ts` の `NODE_KIND_VALID`) に
   * 加えて parts identifier (arc-gauge 等) を accept する。
   * 未知 kind 値は parts 候補として partId に格納、 compile 側 partsCatalog で解決。
...
export interface JsonStep {
   ...
   *
   * 三角 (継承・実装) / 菱 (集約・コンポジション) / 開いた矢 (関連・依存) / 鳥の足 (多)。
   * 書かなければ従来どおり塗った三角になる。
   */
  head?: EdgeHead;
 ...
 * (CAR-1657 の unified syntax)。 つまりこの集合が小さいほど、**書いた種類が箱の見た目に効かなくなる**。
 * 部品を持つ一覧を渡した組み立てでは一覧に無い名前を知らせる (`part-not-found`、#2113) が、
 * 一覧を渡さない組み立てと部品を持たない一覧の組み立てでは `console.warn` にしか残らない。
 *
 * 以前はここに一覧を写していたため、記法に種類が増えても JSON 経路が古い一覧のまま
 * 見本の名前として扱った (実測 = 記法が知る 108 種のうち 77 種が該当、`shape-*` は全滅)。
 * 当時はカタログに無い名前が `console.warn` にしか残らず `onNotice` を呼ばなかったため、
