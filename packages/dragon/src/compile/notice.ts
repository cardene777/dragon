...
export type CompileNotice = {
  kind:
    ...
    // 既定と同じ向きを書き、その向きを外しても同じ組み立てを通る (#2421)。 書いた行が図を
    // 1 ピクセルも動かさない形で、書き手には効いたのか効かなかったのかが画面から読めない。
    // **経路が切り替わる形とは別物** = 向きを書くとフローは鎖をやめ、担当の図は共通の組み立てへ
    // 回る。 その切替が起きる図では既定と同じ値でも絵が変わるので、この知らせは出さない
    ...
    // 同じ図種で、書いた矢印を描かなかった (#1154)。 関係を描かない図種と、端を描けない矢印 (ガントチャートで
    // 時期の無い項目を指す等、#2111)
    | "chart-edge-dropped"
    ...
    | "band-actor-missing"
    // 順序図の帯に書いた区間が、 板に載る言づての並びに収まらない (#2404)。 範囲外と、
    // 始まりが終わりより後ろ の 2 形。 **`row` は言づての行** = 描く側が `rowY(i)` で
    // i 通目の言づての高さを返す。 段 (`animation` の `step`) ではないので `step` と呼ばない
    | "band-row-out-of-range"
    ...
    // 矢印に、その図種が描かない飾り (文字 / 色 / 線種 / 多重度) を書いた (#2111)。 ガントチャートの矢印は
    // 前後の関係だけを使う
    | "edge-option-not-honored"
    // ガントチャートの項目の終わりが始まりより前か、状態で決まる終わりが始まりより前の値を取る (#2111)
