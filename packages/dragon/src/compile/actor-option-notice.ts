...
export const 効かない箱の欄の呼び名 = [
  ["kind", "種類"],
  // 呼び名 (#2390)。 板 / 木 / 骨組みの図は箱に描くので、伝えるのは値として読む図だけ
  ["subtitle", "呼び名"],
  ["posW", "大きさ"],
  ["posH", "大きさ"],
  ["posX", "位置"],
 ...
 * | 区分 | 欄 |
 * |---|---|
 * | どの図種も読む | 名前 / 題 / 値 と、書いた場所と種類を書いたかの印 |
 * | 別の知らせが受け持つ | 縦列 / 位置のずらし / 体験と工程の 4 欄 |
 *
 * 呼び名 (`subtitle`) は除かない (#2390)。 値を書いていない箱で **値として読まれる**
 * (`a.value ?? a.subtitle`) ので除いていたが、**値を書いた箱では読む先が無い**。
 * 除いた結果、値と呼び名を両方書いた箱では誰も伝えず、13 図種で黙って消えていた。
 * 値を書いていない箱で鳴らさない判定は呼び元が持つ = 箱 1 つごとに決まる条件なので、
 * 図種ごとの除外では表せない (#2384 の色番号と同じ形)。
 *
 * 位置 (`posX` / `posY`) は除かない = この図種では両方書いても効かないため、
 ...
 * 実測 = フロー 447 枚 ・ 泳路の図 35 枚 ・ 配置の図 8 枚の見本すべてで、この 4 件が
 * 図も変えず知らせも出さなかった。
 *
