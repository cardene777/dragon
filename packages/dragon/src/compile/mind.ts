...
/**
 * 記法の `type: mind` を engine の `mind-map` 種別の 1 箱に組む (#1177)。
 *
 * 以前は `card` を 3 列 (`mind-left` / `mind-center` / `mind-right`) に並べる別実装で、
 * engine の `mind-map` を使っていなかった。 絵は 3 列の箱から中心の放射に変わり、 破壊的変更
 * として `CHANGELOG` に記録している。
 *
 * 記法の `actors` は「1 つ目が中心、 残りが枝」 の並び。 枝の親は矢印で決まり (#1251)、
 * 書かなければ全ての枝を中心の直下に置く。 描けない欄と、 親にできなかった矢印は黙って
 * 捨てず、 書いた人に伝える。
 */
