 ...
 *
 * 表に書き足す時と版を切る時に守ることは、表の直前の注記が持つ。
 *
...
const 利用者から見えない: Record<string, string> = {
  ...
  //
  // **版を切る時に、範囲の外に出た番号をここから外す**。 範囲は前の版の tag から HEAD までで、
  // 版を切るたびに縮む。 外さないと「宣言が実物の範囲に残っている」 検査が落ちる = 使われない
  // 宣言が溜まると、宣言そのものが空文になるため。
  // どの版で何件外したかは git の履歴が持つので、ここには書き足さない。
  //
  // **版を切る時に tag を打つ**。 打たないと範囲が前の版のまま縮まず、外す対象が現れない。
  // 0.19.0 で打ち忘れており、次の版を切る時に気付いて打ち直した。
  "2276": "版を切る commit そのもの。 変更履歴の更新が中身",
  // 宣言を足す取り込み自身も画面に出ないため、自分の番号も同じ PR で宣言する。
  // 書かないと、取り込むたびに次の取り込みで本 file の検査が落ちる (#2119 / #2121 で 2 回起きた)
  // 取り込みの件名は「PR の題 + PR 番号」 に置き換わる。 題に Issue 番号を書かないと
  // 件名が名乗る番号は PR 番号だけになり、Issue 番号しか書いていない変更履歴から辿れない
  // (#2126 で踏んだ形)。 直す側は変更履歴に PR 番号を書き足すだけなので画面に出ない
  "2268": "携帯の幅で図の中の文字が何 px で描かれるかを測る検査を足しただけ。 測る側を置いただけで、記法も図も画面も 1 文字も変わらない (画面を直したのは #2269 で、そちらは変更履歴に書いた)",
  "2270": "横はみ出しを見る検査の経路を、手で並べた一覧から画面の実装を読む形に置き換えただけ。 見る画面が 10 件から 38 件に増えたが、記法も図も画面も 1 文字も変わらない",
  "2274": "見本の複雑な版が増えた分だけ古くなっていた検査 2 本の固定値を、実物から導く形と実測値に直しただけ。 開く見本も判定も前後で同じで、記法も図も画面も 1 文字も変わらない",
};
  ...
  describe("番号を拾う行 (#1279)", () => {
    ...
    it("番号の後に注記が付いた元件名も拾う", () => {
      ...
      const m = [
        ...
        "",
        "* 📝 docs(spec): animation の注記を直す (#1208 Round 4)",
