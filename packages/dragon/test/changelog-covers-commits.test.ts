...
const 利用者から見えない: Record<string, string> = {
  ...
  "2279": "携帯の幅の読みやすさを見る検査が、描き出しの動きの途中を測っていたのを静止した姿で測る形に直し、古い宣言を 1 件外しただけ。 図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない (画面が変わるのは #2280 で、そちらは変更履歴に書いた)",
  "2283": "携帯の幅の読みやすさを見る検査が、測る前に頁を送るようにしただけ。 見える所まで来て初めて描かれる図 10 枚が母数に入り、下限を割る 8 件を実測値付きで宣言した。 図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない (画面を直すのは #2284)",
  "2277": "配る中身が変わらないまま版を切れてしまう形に、切る前に落ちる検査を足しただけ。 過去 2 版を理由つきで宣言した。 記法も図も画面も 1 文字も変わらない",
  "2293": "部品の縦の矢印が箱の上の読み取り値を割っていることを測る検査を足しただけ。 割れている 4 件を実測つきで宣言した。 直す側は記法の engine 側で動かせないので (#2200)、図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない",
  "2295": "画面の検査の下ごしらえに、開発 server の相手が dragon かを見る判定を足しただけ。 別の画面が居る時に 45 秒の時間切れではなく直し方が出るようになった。 図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない",
  "2289": "検査の中の宣言 1 件の行き先を、決まっていない状態から記法の engine の Issue に書き換えただけ。 直す側は engine にあるので、図も画面も 1 文字も変わらない (測った値も 7.55px のまま)",
  "2298": "調べ物の頁に 2 枚目の図を渡すと前の図を測ってしまう罠を、検査の呼出側に説明として書いただけ。 判定は 1 つも変えておらず、記法も図も画面も 1 文字も変わらない",
  "2302": "線の交差を数える走査が 14 頁のうち 8 頁しか見ていなかったのを、全頁に広げただけ。 交差のある図 4 件を理由つきで宣言し、漏れたら落ちる検査を足した。 図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない",
  "2310": "記法の検査が出す軽い違反 (warn) を軸と図の名前で宣言し、突き合わせる検査を足しただけ。 44 件のうち 3 件しか見えていなかった。 図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない",
  "2320": "訳し忘れを見る検査の注釈から、動く件数を落としただけ (443 と書いてあり実物は 475 だった)。 判定は 1 文字も変えておらず、記法も図も画面も 1 文字も変わらない",
  "2318": "動く見本を分ける表が片方向しか見ておらず、表示部品を持たない図 4 件が 6 つの検査の外に居たのを、図から導いた一覧と両方向で突き合わせる形にしただけ。 同じ区分の一覧を 2 つの検査が別々に持って 1 件食い違っていたのも 1 つに寄せた。 図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない",
  "2316": "見回りの軸を手で並べた一覧を engine の軸と両方向で突き合わせる形にしただけ。 片方向しか見ておらず 16 軸が判定の外に居た (engine 67 / 一覧 51)。 冒頭に書いてあった軸の数 3 つも落とした。 図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない",
  "2314": "「カタログの全図」 を名乗る走査 6 本が群を手で並べていたのを、走査で導く形にしただけ。 10 群から 12 群しか見ておらず、最大で 584 図のうち 176 図が対象外だった。 走査した群が dir の実体と一致することを各走査が見る。 図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない",
  "2308": "図を配置した時に engine が出す知らせを集め、理由つきの宣言と突き合わせる検査を足しただけ。 いま出ている 2 種類 117 行に理由と行き先を付けた (名札と線の 3 件は cardene777/cdl#872 待ち)。 図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない",
  "2306": "線の交差を数える式が円弧の引数を座標として読む形を直しただけ。 道筋に無い点 (11,11) と (0,0) が折れ線に出ていた。 宣言の箇所の数 2 件を実測に合わせた。 図を描く側は 1 行も変えておらず、記法も図も画面も 1 文字も変わらない",
  "2303": "版が配った中身を測る範囲を、版に tag があるかで分けただけ。 版を切った後に入れた直しが切り終えた版の中身に混ざる形と、版を切った後に commit すれば関門が黙る形の両方を塞いだ。 検査だけの変更で、記法も図も画面も 1 文字も変わらない",
};
