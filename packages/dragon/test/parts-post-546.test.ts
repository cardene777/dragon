...
describe("iter546: parts additional (Array group / reduce advanced)", () => {
  it("count", () => { expect(ALL_PARTS.length).toBeGreaterThanOrEqual(60); });
  for (const { name, diagram } of ALL_PARTS) {
    it(`${name}: groupBy via reduce`, () => { const g = [1, 2, 3, 4].reduce<Record<string, number[]>>((acc, n) => { const k = n % 2 === 0 ? "even" : "odd"; (acc[k] ??= []).push(n); return acc; }, {}); expect(g.even).toEqual([2, 4]); expect(g.odd).toEqual([1, 3]); });
    it(`${name}: count via reduce`, () => { const c = ["a", "b", "a"].reduce<Record<string, number>>((acc, x) => { acc[x] = (acc[x] ?? 0) + 1; return acc; }, {}); expect(c.a).toBe(2); expect(c.b).toBe(1); });
