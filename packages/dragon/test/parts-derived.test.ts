...
describe("見本の値を引き継ぐ (#1180)", () => {
  ...
  it("数字で始まる actor 名でも、書き換えた式が読める形になる", () => {
    // 前置きが付くと名前が数字始まりになることがある (`1p__v`)。 engine は裸の名前を
    // `[A-Za-z_$]` で始まる形でしか読まないため、裸で書くと読めない式になる
    const d = 組む({ 見本たち: [{ alias: "1p", part: 見本() }] });
    expect(d.derived?.find((x) => x.id === "1p__half")?.expression).toBe("({1p__v} / 2)");
    expect(描画が読む値(d)["1p__half"]).toBe("20");
  });
...
describe("名前に使えない文字を含む actor 名では、止まったことを伝える (#1180 / #1189)", () => {
  it("英数字でない actor 名に重ねた見本の値は解けず、理由が伝わる", () => {
    // 重ねた先の名前は `{actor 名}__{見本の名前}` で作る。 actor 名が英数字と `_` の外に
    // 出ると engine が名前として読めず、値が止まる。
    //
    // **止まること自体は今回の範囲外** (名前の作り方は状態と箱の id も共有しており、
    // 変えると id の規約ごと動く = `#1189`)。 ここで固定するのは「黙って消えない」 こと。
    ...
    expect(描画が読む値(d)["受付 1__half"]).toBeUndefined();
    const 知らせ = 届いた.find((n) => n.actor === "受付 1__half");
    expect(知らせ?.kind).toBe("value-unresolved");
    expect(知らせ?.message).toContain("名前として使えない");
  });

  it("英数字の actor 名では今まで通り解ける", () => {
    // 上の検査だけだと、全ての見本の値を止める形でも通ってしまう
