...
/**
 * 静止したフローの鎖は、どの行にも書かれていない部品を飛ばす (#1987)。
 *
 ...
 */
describe("静止したフローの鎖は、行に現れない部品を飛ばす (#1987)", () => {
  /** 登場人物の並びを差し替えた静止したフロー */
  const フロー = (登場人物: string[], 流れ = ""): string =>
    `title: "t"\ntype: flow\n\nactors:\n${登場人物.map((a) => `  - ${a}`).join("\n")}\n${流れ ? `\nflow:\n${流れ}\n` : ""}`;
  ...
  it("部品を指さない行は、部品を飛ばした鎖の矢印に載り、端の知らせが出ない", () => {
    ...
    const 図 = textDslToDiagram(
      フロー([受付, 印, 出荷], '  - 受付 -> 出荷: "出す" { head: none }'),
      {
        partsCatalog: 部品の一覧,
  ...
  it("部品だけのフローは矢印を作らず、中身の無い縦列を残さない", () => {
    const { 端, 図 } = 組み立てる(フロー([印, "灯: { kind: three }"]));
    expect(端).toEqual([]);
