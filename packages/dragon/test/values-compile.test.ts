...
describe("値の反例 6 種 (#1162)", () => {
  ...
  it("object の予約名 (`__proto__`) を状態名にすると、組み立てと描画が同じ結果になる", () => {
    // 状態の表は組み立ても描画も **継承を持たない入れ物** で作るため、この名前は
    // どちらでも普通の名前として扱われる。
    //
    // **揃っていることが要点**。 片方だけ拾える形にすると「知らせは出ないのに箱には
    // `{__proto__}` が出る」 状態ができ、本 Issue が消そうとしている食い違いが復活する。
    //
    // 揃える先は engine が決める。 engine が通常の object で組んでいた頃は両方とも
    // 落ちていたが、継承なしに揃えた (cdl#456 / cdl#490) ため両方とも解けるようになった。
    // cdl 0.9.0 へ上げた時点で組み立て側だけが取り残され、実際に食い違っていた
    // (実測 = 描画は `a = 5`、知らせは `value-unresolved`)
    const src = 記法('states:\n  __proto__: 4\n\nvalues:\n  a: "{__proto__} + 1"');
    expect(描画が読む値(src).a).toBe("5");
    // 解けたのだから知らせは出ない。 出ていたら「知らせと箱が食い違う」 に戻っている
    expect(知らせ(src).find((n) => n.actor === "a")).toBeUndefined();
  });

  it("普通の名前と `__proto__` で結果が変わらない", () => {
    // 上の検査だけだと、`__proto__` を特別扱いして 5 を返す実装でも通る。
    // 同じ式を普通の名前で書いた図と突き合わせる
    const 予約名 = 記法('states:\n  __proto__: 4\n\nvalues:\n  a: "{__proto__} + 1"');
    const 普通 = 記法('states:\n  x: 4\n\nvalues:\n  a: "{x} + 1"');
    expect(描画が読む値(予約名).a).toBe(描画が読む値(普通).a);
    expect(知らせ(予約名).length).toBe(知らせ(普通).length);
  });
