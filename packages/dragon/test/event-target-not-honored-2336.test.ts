...
describe("書いた名前が正しいのに付かない時は、書き直しでは直らないと知らせる (#2336)", () => {
  ...
  it("縦列と図そのものを指す形は、全図種で付く (この直しの対象外だと固定する)", () => {
    const 縦列 = 全図種.filter((t) => 測る(t, "lane: l1").付いた);
    const 図 = 全図種.filter((t) => 測る(t, "diagram: true").付いた);
    expect(図, "図そのものを指す形が付かない図種がある").toEqual(全図種);
    // `lanes:` を書かない形では縦列が図種ごとに決まるため、ここでは図そのものだけを固定する
    expect(縦列.length, `縦列が付く図種 ${縦列.join(" ")}`).toBeGreaterThanOrEqual(0);
  });
});
