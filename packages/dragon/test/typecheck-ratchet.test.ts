...
describe("検査 code の型の誤りを減らす方向にだけ動かす (#1413)", () => {
  ...
  /*
   * **待ち時間を明示する** (Round 1 の指摘)。 `tsc` は単独なら 2 秒で終わるが、全件を並列で
   * 回すと負荷でその倍以上かかりうる。
   *
   * 待ち時間で落ちると「誤りが増えた」 と読めない形で失敗し、原因が天井から離れる。
   * 30 秒は実測 (2 秒) の 15 倍で、負荷の振れ幅を吸収する。
   * 一式の既定も同じ 30 秒に揃えた (`vitest.config.ts`、#2087)。 ここに残すのは、
   * この検査が何秒を前提にしているかを その場で読めるようにするため。
   */
  it("残っている誤りが天井と一致する", () => {
