...
export const 受け入れた一覧: readonly 受け入れた群[] = [
  ...
  },
  {
    群: "横に並べた流れの図",
    理由:
      "#2424 でフローの既定を横にした。 4 段を横に並べると幅が出る。 " +
      "縦に戻せば収まるが、それが元の不具合 (縦に長すぎて一覧に収まらない)。 " +
      "段を減らすと見本の中身が変わる。 " +
      "#2426 で箱の幅を 320 から 180 に絞り、幅が 2536 から 2066 になった。 " +
      "見本の頁 (拡大) は境の内側に入り、一覧の器だけ残っている",
    図: [
      { id: "flow-demo", 器: ["一覧"] },
      { id: "認証の流れ", 器: ["一覧"] },
      { id: "値どうしの関係を書く例", 器: ["一覧"] },
  ...
  {
    群: "持ち場を渡って戻る複雑な帯図 (#2169)",
    理由:
      "利用者 / 受付 / 在庫 / 支払 の縦列 4 本に 9 箱を置く (viewBox 2362x630、拡大で題が 10.7px /" +
      " 一覧で 8.1px)。 縦列の幅 (`laneWidth`) は 320 で頭打ちで、260 に下げても 2362 から動かない" +
      " (実測)。 縦列を減らすと持ち場を渡って戻る道が描けない。 持ち場の渡りを横の位置で見せること" +
      "自体が帯図の見本の中身",
    図: [{ id: "swim-complex-demo", 器: ["拡大", "一覧"] }],
  },
  {
    群: "4 つの枠で分けた複雑な構成図 (#2167)",
    理由:
      "利用者側 / 入口 / 動かす場所 / しまう場所 の 4 枠に 10 箱を置く " +
      "(viewBox 2364x916、拡大で題が 10.7px / 一覧で 8.1px)。 枠の幅 (`groupWidth`) は 320 で" +
      "頭打ちで、280 / 240 に下げても 2364 から動かない (実測)。 枠を減らすと役目の分かれ目が" +
      "消えるので減らさない。 枠で役目を分けること自体が構成図の見本の中身",
    図: [{ id: "topo-complex-demo", 器: ["拡大", "一覧"] }],
  },
  {
    群: "回線と壁と交換機を二重にした複雑なネットワーク図 (#2165)",
    理由:
      "役目ごとに列を分けて 4 列 3 段に置く (viewBox 2389x615、拡大で題が 10.6px / 一覧で 8.0px)。" +
      " 幅は列の数だけで決まり、縦列の幅 (`laneWidth`) を 260 / 200 に下げても、機器の題を 3 字に" +
      "縮めても 2389 から動かない (実測)。 二重にした経路を横に並べて見せること自体が見本の中身",
    図: [{ id: "network-complex-demo", 器: ["拡大", "一覧"] }],
  },
  {
...
export const 縦に長い一覧: readonly 縦に長い群[] = [
  ...
  },
  {
    ...
    図: [
      // 左と右を上下に置き、そろった分をその真ん中の段に置く (#2154)。 出口を入口と同じ段に
      // 置くと、描画側が同じ辺に入る 2 本の終点を離すため線が出口の手前で段を作る
      "parts-barrier-box",
      // 置ける分と押し出す分を上下に置き、届く箱を真ん中の段に置く (#2199)。 横に並べると
      // 置き場より押し出す方が多いことが棒の高さで読めなくなる
      "parts-evict-box",
      "parts-battery-level",
      "parts-bind-cascade-3",
      "parts-bind-split-fill",
      "parts-bookmark",
      // まだ残る分と消える分を上下に置き、置く箱を真ん中の段に置く (#2199)。 横に並べると
      // 時が過ぎて減ったことが棒の高さで読めなくなる
      "parts-expire-box",
      // 手元から返す分と奥へ行く分を上下に置き、問い合わせを真ん中の段に置く (#2193)。
      // 横に並べると、多い方と少ない方の棒の高さを見比べられなくなる
      "parts-cache-box",
      // 通る分と落ちる分と断る分を縦に並べる (#2193)。 3 つを同時に見せないと
      // 「落ちる分が増えたから断りに回った」 という順番が読めない
      "parts-circuit-breaker",
      // 中身の種類で 3 つの行き先へ分ける形が図の中身 (#2171)。 3 つの棒の高さが揃わないことを
      // 見せるので、行き先を 1 つでも減らすと「種類で変わる」 が読めなくなる
      "parts-content-sorter",
      // 残す分と捨てる分を上下に置き、届く箱を真ん中の段に置く (#2193)。 横に並べると
      // 出る数が入る数より少ないことが棒の高さで読めなくなる
      "parts-dedupe-box",
      // 受け手 3 つの棒が発行と同じ高さまで伸びる形が図の中身 (#2157)。 3 つを縦に並べるので、
      // 入口を真ん中の段に置くと図が 3 段になる
      "parts-fanout-copy",
      // 常用と予備を上下に置き、入口をその真ん中の段に置く (#2171)。 振り分け器と同じ置き方に
      // することで、「両方へ分ける」 と「どちらか一方へ倒す」 を棒の高さだけで見比べられる
      "parts-failover-switch",
      ...
      "parts-rainbow-stack",
      "parts-rating-stars",
      // 出せる分と待つ分を上下に置き、届く箱を真ん中の段に置く (#2206)。 横に並べると
      // 出せない分が下に溜まることが棒の高さで読めなくなる
      "parts-reorder-box",
      // 同じ鍵の先と別の鍵の先を上下に置き、届く箱を真ん中の段に置く (#2206)。 振り分け器と
      // 同じ置き方にすることで、割合で決まる形と鍵で決まる形を棒の偏りだけで見比べられる
      "parts-key-router",
      // 奥へ行く本数と皆へ配る件数を上下に置き、問いを真ん中の段に置く (#2206)。 同じ上限の
      // 棒を縦に並べないと、24 件の問いが 2 本になったことが読めない
      "parts-single-flight",
      // 送り主 2 人を上下に置き、出口をその真ん中の段に置く (#2154 と同じ理由、#2206)。
      // 横に並べると、たくさん送る人の棒が満杯のまま動かないことが読めなくなる
      "parts-fair-queue",
      // 先へ進む分と脇へ出す分を上下に置き、やり直す箱を真ん中の段に置く (#2218)。 横に並べると
      // 脇へ出た 3 件が本筋へ戻らないことが、棒の高さの差として読めなくなる
      "parts-dead-letter",
      // 中に入る分と待つ分を上下に置き、来る箱を真ん中の段に置く (#2218)。 同じ上限の棒を縦に
      // 並べないと、18 件のうち中に居るのが 1 件だけであることが読めない
      "parts-lock-gate",
      // そのまま先へ行く分と控えに取る分を上下に置き、通る箱を真ん中の段に置く (#2220)。
      // 同じ上限の棒を縦に並べないと、入口と本筋が同じ高さのままであることが読めない
      "parts-sample-tap",
      // すぐ返す分とあとで片づく分を上下に置き、頼まれる箱を真ん中の段に置く (#2220)。
      // 横に並べると、返事が入口と同じ高さで片づけだけ低いことが読み取れなくなる
      "parts-ack-first",
      // 小さい荷物と札を上下に置き、大きい荷物を真ん中の段に置く (#2199)。 上限の違う棒を
      // 横に並べると 3 個が 24 個になることが読めなくなる
      "parts-split-box",
      // 通る箱に 2 本が入るので、試す箱とやり直す箱の真ん中の段に置く (#2171)。 2 段で組むと
      // 線が通る箱の手前で長さ 28 の縦の段を作る (実測、#2154 と同じ形)
      "parts-retry-loop",
  ...
  },
  {
    群: "縦に積む向きの見本 (#2524)",
    理由:
      "並べる向きを選べることを見せる図で、縦に積むこと自体が見本の中身。" +
      " 横に並べた版 (`flowchart-demo`) が同じ 4 箱で隣に在り、見比べるための対になっている。" +
      " 縮めるには箱を減らすしかなく、減らすと分岐が消えて分かれ道の図でなくなる",
    図: ["flowchart-vertical-demo"],
  },
  {
    群: "部品を箱に使う見本 (#2004)",
    理由:
