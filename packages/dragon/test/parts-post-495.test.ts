...
describe("iter495: parts additional (Regex basic)", () => {
  ...
  for (const { name, diagram } of ALL_PARTS) {
    ...
    it(`${name}: /^abc$/.test("abc") = true`, () => { expect(/^abc$/.test("abc")).toBe(true); });
    it(`${name}: id matches non-empty`, () => { if (diagram.id.length > 0) expect(/.+/.test(diagram.id)).toBe(true); else expect(true).toBe(true); });
  }
});
