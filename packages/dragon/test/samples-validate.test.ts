/**
 * Editor SAMPLES 12 diagram の cdl validate check (CAR-1659)。
 *
 * user report = 「サンプルで cdl validate failed: phase が 0 件です」 error が出るものがある。
 * 12 sample の text DSL を textDslToDiagram + compile で machine check、 error 出す sample を
 * 特定する。 fix 後は「全 12 sample が validate pass」 assert で regression 検知。
 *
...
describe("editor SAMPLES 12 diagram validate", () => {
  it("SAMPLES count が 12 (期待値、 CdlEditor.tsx 側と drift しない SSOT check)", () => {
