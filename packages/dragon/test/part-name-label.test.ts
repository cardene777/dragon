 ...
 * | 形 | 直す前 (実測) |
 * |---|---|
 * | 部品 4 つだけのフロー | `稼働2` の名札が `稼働3` の上、`稼働4` の名札が `稼働2` の上 |
...
describe("部品の名前の名札は、その部品のすぐ上に出る (#1990)", () => {
  it("部品 4 つだけのフローで、各部品の名札の縦列が部品を横に含み、部品の上端から 60 上にある", () => {
...
describe("固定した部品用の縦列は、部品の要素を縦に収め、図の下の空きを変えない (#1990)", () => {
  it.each([
    ["部品 4 つだけのフロー", 部品だけ(4), ["稼働1", "稼働2", "稼働3", "稼働4"]],
    [
