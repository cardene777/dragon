 ...
 *
 * そこで #1440 で「**描く見本は全段を描く段の長さに揃える**」 に変えた。 対象は `draw` を
 * 持つ見本すべてで、描く段を持たない見本は従来どおり既定のまま。
 *
 * 下の 2 件はその契約変更に合わせて書き換えてある (削除ではなく、主張の載せ替え)。
 */
...
describe("見本ごとに段の長さを書ける (#1353)", () => {
  ...
  it("折れ線の 2 段目は 1 段目と同じ長さ (#1440 で契約が変わった)", () => {
    // 旧: 「長さを書いていない段は既定のまま」。 `描き直す` が 2 段目も描く段にするため、
    // 既定のままだと描き直しが 1 段目より 2.7 倍速くなる (#1440)
    const 段 = 折れ線().phases;
    expect(段.length, "2 段目が無い").toBeGreaterThan(1);
    expect(段[1]!.duration).toBe(描く段の長さ);
    // 既定と同じ値では「揃えた」 ことにならない = 伸ばしたことを直接見る
    expect(段[1]!.duration, "2 段目が既定より長くない").toBeGreaterThan(既定の長さ);
  });
  ...
  it("長さを伸ばしたのは、起点から描く見本の段だけ (#1440 で契約が変わった)", () => {
    // 旧: 「伸ばしたのは描く段だけ」。 `描き直す` は 2 段目も描く段にするため、その段も
    // 揃える必要がある (#1440)。 描く段を持たない見本まで伸ばしていないことは引き続き見る
    const 長い: string[] = [];
    const 描く見本の段: string[] = [];
    for (const { name, diagram } of 図の一覧()) {
      const 描く見本 = diagram.phases.some((p) => (p.draw ?? []).length > 0);
      for (const p of diagram.phases) {
        if (p.duration > 既定の長さ) 長い.push(`${name}:${p.id}`);
        if (描く見本) 描く見本の段.push(`${name}:${p.id}`);
      }
    }
    expect(描く見本の段.length, "描く見本が 1 つも無い (検査が空振りしている)").toBeGreaterThan(0);
    expect(長い.sort()).toEqual(描く見本の段.sort());
  });
});
