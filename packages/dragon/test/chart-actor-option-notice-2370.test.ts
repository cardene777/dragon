/*
 * ガントチャートと体験の地図と四象限で、箱に書いた指定が黙って消えないことを見る (#2370)。
 *
 ...
 * ## 読む欄で鳴らさない
 *
 * 図種ごとに読む欄が違う (ガントチャートは色味と担当と終わる時期、体験の地図は接点と伸びしろ)。
...
describe("ガントチャートと体験の地図と四象限で、箱に書いた指定が黙って消えない (#2370)", () => {
  ...
  it("箱を持つ図種では、伝える欄がはっきり少ない (植え込み対照)", () => {
     ...
     * 「0 件」 では対照にならない。 ガントチャートが伝える欄の数がフローより真に多いことを見る。
...
`).知.some((n) => n.kind === "actor-option-not-honored"),
    ...
    const ガントチャートで伝える = 項目.filter((k) =>
      組む(本文(図種[0]!, `${k}: ${値[k]}`)).知.some((n) => n.kind === "actor-option-not-honored"),
    );
