...
const 見逃す組の一覧: readonly 見逃す組[] = [
  ...
  // 位置を相対で書く見本 (#2039)。 自動の配置は同じ縦列の箱を縦 1 列に揃えるが、この記法は
  // **その並びから外して置く** ためにある。 揃えの指摘はその 1 点を指しており、揃え直すと
  // 見本が何も見せなくなる。
  //
  // 横にずらす向き (右 / 左) の図だけが出る。 縦にずらす向き (上 / 下) は中心の横位置が
  // 変わらないので指摘にならない (実測 = 4 図のうち 2 図だけが出た)。
  //
  // 「部品を箱に使う」 の頁の同じ形 (部品を基準にして横へ並べる) は出ない。 3 つの箱がどれも
  // 相対で置かれており、揃える基準になる縦列の並びが最初から作られないため (実測)。
  ...["受付の右に確認を置く", "受付の左に確認を置く", "間隔を書かずに受付の右へ置く"].flatMap(
    (diagramId) =>
      ["alignment", "column-alignment"].map((axis) => ({
        diagramId,
        axis,
        理由: "自動の配置の並びから外して置くこと自体が、位置を相対で書く見本の意図",
      })),
  ),
];
...
describe("名指しした見逃しが実物で当たっている (#1730)", () => {
  ...
  it("error の行に engine が直せなかった理由を足す (理由を持たない違反は従来の行のまま)", () => {
    /*
     * カタログには今 `clearance` の error が無く、 実物からは理由を持つ違反を採れない。
