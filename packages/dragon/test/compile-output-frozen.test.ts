 ...
 *
 * **基準にする部品は位置を書かない見本にする**。 相対で書いた部品は組み立ての段階で座標が
 * 決まっておらず、`partBoxes` が「後で埋める」 として飛ばす = 相対で置いた部品だけを基準に
 * すると、この網は `partBoxes` に届かないまま通る (#2039 で実測)。 位置を書かない部品は格子に
 * 置かれるので範囲が決まり、基準にできる。
 *
 * #2039 では基準に座標を書いていた。 格子の起点が相対で置く箱の分だけ後から動き、縦の位置が
 * 揃わなかったため (実測で基準と箱の中心が 464 離れた)。 #2041 で起点から相対で置く箱を外した。
 */
