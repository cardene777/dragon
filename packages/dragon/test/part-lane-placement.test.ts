...
/**
 * 入口 1 つを出口 2 つへ分ける部品 (#2145)。 カタログの `parts-split-router` を 3 段に揃える前の形
 * (#2154) = 左の縦列に入口、右の縦列に出口を 2 つ縦に積み、部品の中に矢印を 2 本持つ。 縦列ごとに
 * 段の数が違う部品を置いても箱が揃うかを見るので、段の数が違うこの形のまま残す
 */
const 分岐: CdlDiagram = {
  id: "parts-split2",
  topic: "split2",
