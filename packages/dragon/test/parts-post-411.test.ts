...
describe("iter411: parts additional (TextEncoder/TextDecoder)", () => {
  it("count", () => { expect(ALL_PARTS.length).toBeGreaterThanOrEqual(60); });
  for (const { name, diagram } of ALL_PARTS) {
    it(`${name}: new TextEncoder().encode("abc").length = 3`, () => { expect(new TextEncoder().encode("abc").length).toBe(3); });
    it(`${name}: new TextDecoder().decode(new Uint8Array([97,98,99])) = "abc"`, () => { expect(new TextDecoder().decode(new Uint8Array([97, 98, 99]))).toBe("abc"); });
