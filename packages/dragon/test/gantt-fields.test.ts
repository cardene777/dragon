...
describe("工程の並びに担当と終わる時期を書ける (#1251)", () => {
  ...
  it("目盛りに無い時期を書いたら始まりと同じに倒し、読めないことを伝える (#2392)", () => {
    // 目盛りは書かれた順に作るため、載っていない名前は位置を持たない。
    // 倒すだけだと「書いたのに 1 コマのまま」 になり、書いた人に理由が見えない
    expect(帯一覧(工程(', end: "Q9"'))?.[1]?.endIdx).toBe(1);
    const 知 = 組み立てる(工程(', end: "Q9"')).知らせ.filter(
      (n) => n.kind === "chart-value-unreadable",
    );
    expect(知.length, "目盛りに無い時期が黙って消えている").toBe(1);
    expect(知[0]?.message, `文 "${知[0]?.message ?? ""}"`).toContain("Q9");
    // 目盛りは図ごとに変わるので、その図の目盛りを並べる (固定の一覧を書かない)
    for (const 語 of ["Q1", "Q2", "Q3"]) {
      expect(知[0]?.message, `文に目盛り ${語} が無い`).toContain(語);
    }
  });
