 ...
 * | 円弧の引数を座標として読む | #2306 / `cardene777/cdl#872` (0.79.0 で直った) |
 *
 * ## 何を固定して、何を固定しないか
 *
 * | 何を | 固定するか | 理由 |
 ...
 * | `lane-label ∩ edge-path` の件 | **0 件で** | 0.79.0 で engine が直った。 1 件でも出たら戻った合図 |
...
describe("図を配置した時に出る知らせを、宣言と突き合わせる (#2308)", () => {
  ...
  it("名札と線の重なりが 1 件も出ない", () => {
    // 0.78.0 までは 3 件出ていた (`parts-retry-loop` の `rt1` / `rt2` と、
    // 同じ図を記法から書いた分)。 engine が円弧の引数を座標として読み、線の外接矩形が図の
    // 左上まで伸びていたため。 画面では 358px 離れていた。
    //
    // 0.79.0 で engine が命令ごとの引数の数を読むようになり 0 件になった
    // (`cardene777/cdl#872`)。 名指しの宣言は外し、1 件でも出たら戻った合図にする。
    const 出た = 集めた.出た
      .filter((x) => x.種類 === "overlap lane-label ∩ edge-path")
      .map(鍵)
      .sort();
    expect(出た, "離れている名札と線が重なると知らされている").toEqual([]);
  });
});
