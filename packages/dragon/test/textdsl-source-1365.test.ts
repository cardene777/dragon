...
describe("Text DSL のページは記法を持つ (#1365)", () => {
  ...
  it("図を 22 件集められている", () => {
    ...
    // #2344 で箱の項目名を 1 行にまとめて書く変種を足して 21 → 22
    // #2377 で行と印の見本を 2 枚 (英語と日本語) 足して 22 → 24
    // #2396 でクラス図で書く変種を 1 枚足して 24 → 25
    // #2513 で分かれ道の図の見本を 1 枚足して 25 → 26
    expect(図.map((x) => x.key).sort()).toHaveLength(26);
  });
  ...
  it("記法から組み立てた図が、export された図と一致する", () => {
    ...
    expect(照合した, "1 件も照合していない (検査が空振りしている)").toBe(26);
  });
