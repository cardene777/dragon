/**
 * ガントチャート (`type: gantt`) で効かない形を書くと、書いた行を指す知らせが出る (#2111)。
 *
 * ガントチャートの組み立ては効かない形を `console.warn` にだけ出していた。 編集画面は知らせ (`onNotice`) だけを
 ...
 * | 飾り (文字 / 色味 / 線の種類 / 多重度 ほか) を書いた矢印 | `edge-option-not-honored` | その矢印の行 |
 *
 * 呼び名は図種をまたいで 1 つの表から採る (#2366)。 同じ欄を図種ごとに別の文言で呼ぶと、
...
const 効かない形: Array<{ 名: string; src: string; 種類: CompileNotice["kind"]; 印: string; 語: string }> = [
  ...
  {
    名: "時期の無い項目へ引いた矢印",
    src: ガントチャート("actors:", '  - 設計: "Q1"', "  - 実装", "flow:", "  - 設計 -> 実装"),
  ...
  {
    名: "飾りを書いた矢印",
    src: ガントチャート("actors:", '  - 設計: "Q1"', '  - 実装: "Q2"', "flow:", '  - 設計 -> 実装: "後" (info, dashed)'),
  ...
  {
    名: "終わりが始まりより前",
    src: ガントチャート("actors:", '  - 設計: { value: "Q1" }', '  - 実装: { value: "Q3", end: "Q1" }'),
  ...
  },
  {
    ...
    src: ガントチャート("actors:", '  - 設計: { value: "Q1" }', '  - 実装: { value: "Q2", end: "{done}" }', "states:", "  done: 0"),
    種類: "gantt-end-before-start",
    印: "  - 実装",
    語: "始まりより前になる値",
  },
...
describe("ガントチャートで効かない形を書くと、書いた行へ知らせる (#2111)", () => {
  ...
  it.each([
    ["飾りの無い矢印", ガントチャート("actors:", '  - 設計: "Q1"', '  - 実装: "Q2"', "flow:", "  - 設計 -> 実装")],
    /*
     * **終わりの語を目盛りに載せる** (#2392)。 目盛りは箱に書いた時期から作るので、
     * どの箱も Q2 を書いていない図では `end: "Q2"` が目盛りに無い語になり、
     * 帯が伸びないまま「知らせない」 が通っていた (この形がまさに #2392 の穴だった)。
     */
    [
      "終わりが始まりより後",
      ガントチャート(
    ...
    ["終わりが始まりと同じ", ガントチャート("actors:", '  - 設計: { value: "Q1" }', '  - 実装: { value: "Q2", end: "Q2" }')],
    [
      "状態で決まる終わりが始まり以降の値だけを取る",
      ガントチャート("actors:", '  - 設計: { value: "Q1" }', '  - 実装: { value: "Q2", end: "{done}" }', "states:", "  done: 1"),
    ],
  ])("%s: 知らせない", (_名, src) => {
    const { 知らせ, 警告 } = 組み立てる(src);
    expect(知らせ).toEqual([]);
  ...
  it("JSON から書いても、飾りの知らせが 1 件だけ出る", () => {
    ...
    // 多重度の知らせはガントチャートの知らせと同じ 0 行を避ける
    const 知らせ: CompileNotice[] = [];
    const もとの = console.warn;
    console.warn = () => {};
    try {
