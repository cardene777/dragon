...
describe("iter465: parts additional (String indexOf / lastIndexOf / search)", () => {
  ...
  for (const { name, diagram } of ALL_PARTS) {
    ...
    it(`${name}: id indexOf first char != -1 or empty`, () => { if (diagram.id.length > 0) expect(diagram.id.indexOf(diagram.id[0])).toBe(0); else expect(diagram.id).toBe(""); });
  }
});
