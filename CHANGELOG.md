...
- **箱を書いた縦列へ入れられるようにした** (`lane:`、#1266)

  縦列が 2-4 本あり箱がそこに散る図 (構成図 / 処理の流れ / 役割ごとの流れ) を書けなかった。
  縦列を並べるために使う図種 (`flow` / `topology` / `swimlane`) で、**全ての箱に書いた時だけ**
  効く。 一部だけ書くと、書かなかった箱をどこに置くか決められないため知らせが出る。

- **箱の大きさを書けるようにした** (`posW` / `posH`、#1262)

- **矢印の説明文を線の上に重ねられるようにした** (`overlay`、#1267)

  分岐図の条件ラベル (`true` / `false`) 用。

- **2 つの軸で仕分ける図に軸の名前を書けるようにした** (`axes:`、#1255)

- **体験の道筋に場所と改善の余地を書けるようにした** (`touchpoint` / `opportunity`、#1254)

- **工程の並びに担当と終わる時期を書けるようにした** (`owner` / `end`、#1256)

- **図表の箱に上の小見出しを書けるようにした** (`eyebrow:`、#1250)

- **放射の図で枝の親を書けるようにした** (#1253)

- **値がきっかけで動き出す形を書けるようにした** (#1229)

- **JSON の段でも値を動かせるようにした** (#1215)
...
- **記法の `type: mind` を `mind-map` 種別に寄せた** (破壊的変更、#1223)

  従来は `card` を 3 列に並べる別実装で、engine の放射する絵に辿り着けなかった。

### Fixed

#### 書いたのに効かない / 黙って消える形

- **箱に書いた縦列が効かないことを伝えるようにした** (#1248)
...
- **記法で書いた図の大きさを組立て API に揃えた** (#1261)
- **表と状態の図の名前の二重描画を直し、縦列 id を書き直せるようにした** (#1258)
...
### Added (旧 Unreleased から引き継いだ分)

- **箱を書いた縦列へ入れられるようにした** (`lane:`、 #1263)
...
- **工程の並びに担当と終わる時期を書けるようにした** (`owner:` / `end:`、 #1251)
...
- **2 つの軸で仕分ける図に軸の名前を書けるようにした** (`axes:`、 #1251)
...
- **体験の道筋に場所と改善の余地を書けるようにした** (`touchpoint:` / `opportunity:`、 #1251)
...
- **放射の図で枝の親を書けるようにした** (`type: mind` の矢印、 #1251)
...
- **図表の箱に上の小見出しを書けるようにした** (`eyebrow:`、 #1247)
...
- **箱に書いた縦列が効かないことを伝えるようにした** (`lane:`、 #1246)
