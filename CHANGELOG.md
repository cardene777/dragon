...
- **`type: mind` の絵が変わった (破壊的変更、 #1177)**
  ...
  **箱ごとに描いていた頃の欄も描けなくなる**。 1 箱で描く種別が持てるのは名前と枝の色だけで、
  以下は書いても出ない。 書かれていたら伝える。

  | 欄 | 中心 | 枝 |
  |---|---|---|
  | 名前 | 出る | 出る |
  | 色 (`tone`) | **出ない** | 出る |
  | 副題 / 上の小見出し / 値 / 行 | **出ない** | **出ない** |

  これらを描くなら `type: tree` か `type: flow` を使う。

  見本 (`parts`) を重ねた登場人物は枝にせず、 見本の中身だけを描く。 枝にも載せると同じ
  登場人物が 2 箇所に描かれるため。
