...
- **`@cardenelabs/cdl` の依存を `^0.9.0` から `^0.10.0` に上げた** (#1312)

  段が `draw` を持てるようになったのは cdl 0.10.0 から (cdl#513)。 折れ線を伸ばす描画そのものは
  cdl の `kinds/chart-line.tsx` にあり、`pathLength` を 1 に正規化した dash で伸ばす。
  点の数 (`points`) は進みに関わらず変わらないため、点の数を数える既存の検査に影響しない。
